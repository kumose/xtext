...
#ifndef XTEXT_FMT_CHRONO_H_
#define XTEXT_FMT_CHRONO_H_
...
#ifndef XTEXT_FMT_SAFE_DURATION_CAST
#  define XTEXT_FMT_SAFE_DURATION_CAST 1
#endif
#if XTEXT_FMT_SAFE_DURATION_CAST
...
template <typename To, typename From,
          XTEXT_FMT_ENABLE_IF(!std::is_same<From, To>::value &&
                        std::numeric_limits<From>::is_signed ==
                            std::numeric_limits<To>::is_signed)>
XTEXT_FMT_CONSTEXPR auto lossless_integral_conversion(const From from, int& ec)
    -> To {
  ec = 0;
...
template <typename To, typename From,
          XTEXT_FMT_ENABLE_IF(!std::is_same<From, To>::value)>
XTEXT_FMT_CONSTEXPR auto safe_float_conversion(const From from, int& ec) -> To {
  ec = 0;
  using T = std::numeric_limits<To>;
...
template <typename To, typename FromRep, typename FromPeriod,
          XTEXT_FMT_ENABLE_IF(std::is_floating_point<FromRep>::value),
          XTEXT_FMT_ENABLE_IF(std::is_floating_point<typename To::rep>::value)>
...
#ifdef XTEXT_FMT_USE_UTC_TIME
// Use the provided definition.
#elif defined(__cpp_lib_chrono)
#  define XTEXT_FMT_USE_UTC_TIME (__cpp_lib_chrono >= 201907L)
#else
#  define XTEXT_FMT_USE_UTC_TIME 0
#endif
#if XTEXT_FMT_USE_UTC_TIME
...
#ifdef XTEXT_FMT_USE_LOCAL_TIME
// Use the provided definition.
#elif defined(__cpp_lib_chrono)
#  define XTEXT_FMT_USE_LOCAL_TIME (__cpp_lib_chrono >= 201907L)
#else
#  define XTEXT_FMT_USE_LOCAL_TIME 0
#endif
#if XTEXT_FMT_USE_LOCAL_TIME
...
// Usage: f XTEXT_FMT_NOMACRO()
#define XTEXT_FMT_NOMACRO
...
void write_codecvt(codecvt_result<CodeUnit>& out, string_view in,
  ...
  XTEXT_FMT_PRAGMA_CLANG(diagnostic ignored "-Wdeprecated")
  ...
  if (result != std::codecvt_base::ok)
    XTEXT_FMT_THROW(format_error("failed to format time"));
}
...
inline void do_write(buffer<Char>& buf, const std::tm& time,
  ...
  if (end.failed()) XTEXT_FMT_THROW(format_error("failed to format time"));
}
...
XTEXT_FMT_NORETURN inline void throw_duration_error() {
  XTEXT_FMT_THROW(format_error("cannot format duration"));
}

// Cast one integral duration to another with an overflow check.
template <typename To, typename FromRep, typename FromPeriod,
          XTEXT_FMT_ENABLE_IF(std::is_integral<FromRep>::value&&
...
inline auto gmtime(std::time_t time) -> std::tm {
  ...
  if (!gt.run()) XTEXT_FMT_THROW(format_error("time_t value out of range"));
  return gt.tm_;
}
...
XTEXT_FMT_CONSTEXPR inline auto get_units() -> const char* {
...
XTEXT_FMT_CONSTEXPR auto parse_chrono_format(const Char* begin, const Char* end,
                                       Handler&& handler) -> const Char* {
  if (begin == end || *begin == '}') return begin;
  if (*begin != '%') XTEXT_FMT_THROW(format_error("invalid format"));
  auto ptr = begin;
  while (ptr != end) {
    ...
    ++ptr;  // consume '%'
    if (ptr == end) XTEXT_FMT_THROW(format_error("invalid format"));
    c = *ptr;
    switch (c) {
    case '_':
      pad = pad_type::space;
    ...
    case 'E': {
      ...
      default:  XTEXT_FMT_THROW(format_error("invalid format"));
      }
      break;
    }
    case 'O':
...
template <typename Derived> struct null_chrono_spec_handler {
  XTEXT_FMT_CONSTEXPR void unsupported() {
    static_cast<Derived*>(this)->unsupported();
  }
  XTEXT_FMT_CONSTEXPR void on_year(numeric_system, pad_type) { unsupported(); }
  XTEXT_FMT_CONSTEXPR void on_short_year(numeric_system) { unsupported(); }
  XTEXT_FMT_CONSTEXPR void on_offset_year() { unsupported(); }
  XTEXT_FMT_CONSTEXPR void on_century(numeric_system) { unsupported(); }
  XTEXT_FMT_CONSTEXPR void on_iso_week_based_year() { unsupported(); }
  XTEXT_FMT_CONSTEXPR void on_iso_week_based_short_year() { unsupported(); }
  XTEXT_FMT_CONSTEXPR void on_abbr_weekday() { unsupported(); }
  XTEXT_FMT_CONSTEXPR void on_full_weekday() { unsupported(); }
  XTEXT_FMT_CONSTEXPR void on_dec0_weekday(numeric_system) { unsupported(); }
  XTEXT_FMT_CONSTEXPR void on_dec1_weekday(numeric_system) { unsupported(); }
  XTEXT_FMT_CONSTEXPR void on_abbr_month() { unsupported(); }
  XTEXT_FMT_CONSTEXPR void on_full_month() { unsupported(); }
  XTEXT_FMT_CONSTEXPR void on_dec_month(numeric_system, pad_type) { unsupported(); }
  XTEXT_FMT_CONSTEXPR void on_dec0_week_of_year(numeric_system, pad_type) {
    unsupported();
  }
  XTEXT_FMT_CONSTEXPR void on_dec1_week_of_year(numeric_system, pad_type) {
    unsupported();
  }
  XTEXT_FMT_CONSTEXPR void on_iso_week_of_year(numeric_system, pad_type) {
    unsupported();
  }
  XTEXT_FMT_CONSTEXPR void on_day_of_year(pad_type) { unsupported(); }
  XTEXT_FMT_CONSTEXPR void on_day_of_month(numeric_system, pad_type) {
    unsupported();
  }
  XTEXT_FMT_CONSTEXPR void on_24_hour(numeric_system) { unsupported(); }
  XTEXT_FMT_CONSTEXPR void on_12_hour(numeric_system) { unsupported(); }
  XTEXT_FMT_CONSTEXPR void on_minute(numeric_system) { unsupported(); }
  XTEXT_FMT_CONSTEXPR void on_second(numeric_system) { unsupported(); }
  XTEXT_FMT_CONSTEXPR void on_datetime(numeric_system) { unsupported(); }
  XTEXT_FMT_CONSTEXPR void on_loc_date(numeric_system) { unsupported(); }
  XTEXT_FMT_CONSTEXPR void on_loc_time(numeric_system) { unsupported(); }
  XTEXT_FMT_CONSTEXPR void on_us_date() { unsupported(); }
  XTEXT_FMT_CONSTEXPR void on_iso_date() { unsupported(); }
  XTEXT_FMT_CONSTEXPR void on_12_hour_time() { unsupported(); }
  XTEXT_FMT_CONSTEXPR void on_24_hour_time() { unsupported(); }
  XTEXT_FMT_CONSTEXPR void on_iso_time() { unsupported(); }
  XTEXT_FMT_CONSTEXPR void on_am_pm() { unsupported(); }
  XTEXT_FMT_CONSTEXPR void on_duration_value() { unsupported(); }
  XTEXT_FMT_CONSTEXPR void on_duration_unit() { unsupported(); }
  XTEXT_FMT_CONSTEXPR void on_utc_offset(numeric_system) { unsupported(); }
  XTEXT_FMT_CONSTEXPR void on_tz_name() { unsupported(); }
};
...
class tm_format_checker : public null_chrono_spec_handler<tm_format_checker> {
 ...
 public:
  ...
  XTEXT_FMT_NORETURN inline void unsupported() {
    XTEXT_FMT_THROW(format_error("no format"));
  }

  template <typename Char>
  XTEXT_FMT_CONSTEXPR void on_text(const Char*, const Char*) {}
  XTEXT_FMT_CONSTEXPR void on_year(numeric_system, pad_type) {}
  XTEXT_FMT_CONSTEXPR void on_short_year(numeric_system) {}
  XTEXT_FMT_CONSTEXPR void on_offset_year() {}
  XTEXT_FMT_CONSTEXPR void on_century(numeric_system) {}
  XTEXT_FMT_CONSTEXPR void on_iso_week_based_year() {}
  XTEXT_FMT_CONSTEXPR void on_iso_week_based_short_year() {}
  XTEXT_FMT_CONSTEXPR void on_abbr_weekday() {}
  XTEXT_FMT_CONSTEXPR void on_full_weekday() {}
  XTEXT_FMT_CONSTEXPR void on_dec0_weekday(numeric_system) {}
  XTEXT_FMT_CONSTEXPR void on_dec1_weekday(numeric_system) {}
  XTEXT_FMT_CONSTEXPR void on_abbr_month() {}
  XTEXT_FMT_CONSTEXPR void on_full_month() {}
  XTEXT_FMT_CONSTEXPR void on_dec_month(numeric_system, pad_type) {}
  XTEXT_FMT_CONSTEXPR void on_dec0_week_of_year(numeric_system, pad_type) {}
  XTEXT_FMT_CONSTEXPR void on_dec1_week_of_year(numeric_system, pad_type) {}
  XTEXT_FMT_CONSTEXPR void on_iso_week_of_year(numeric_system, pad_type) {}
  XTEXT_FMT_CONSTEXPR void on_day_of_year(pad_type) {}
  XTEXT_FMT_CONSTEXPR void on_day_of_month(numeric_system, pad_type) {}
  XTEXT_FMT_CONSTEXPR void on_24_hour(numeric_system, pad_type) {}
  XTEXT_FMT_CONSTEXPR void on_12_hour(numeric_system, pad_type) {}
  XTEXT_FMT_CONSTEXPR void on_minute(numeric_system, pad_type) {}
  XTEXT_FMT_CONSTEXPR void on_second(numeric_system, pad_type) {}
  XTEXT_FMT_CONSTEXPR void on_datetime(numeric_system) {}
  XTEXT_FMT_CONSTEXPR void on_loc_date(numeric_system) {}
  XTEXT_FMT_CONSTEXPR void on_loc_time(numeric_system) {}
  XTEXT_FMT_CONSTEXPR void on_us_date() {}
  XTEXT_FMT_CONSTEXPR void on_iso_date() {}
  XTEXT_FMT_CONSTEXPR void on_12_hour_time() {}
  XTEXT_FMT_CONSTEXPR void on_24_hour_time() {}
  XTEXT_FMT_CONSTEXPR void on_iso_time() {}
  XTEXT_FMT_CONSTEXPR void on_am_pm() {}
  XTEXT_FMT_CONSTEXPR void on_utc_offset(numeric_system) {
    if (!has_timezone_) XTEXT_FMT_THROW(format_error("no timezone"));
  }
  XTEXT_FMT_CONSTEXPR void on_tz_name() {
    if (!has_timezone_) XTEXT_FMT_THROW(format_error("no timezone"));
  }
};
...
template <typename T, XTEXT_FMT_ENABLE_IF(has_tm_zone<T>::value)>
...
template <typename T, typename Int, XTEXT_FMT_ENABLE_IF(std::is_integral<T>::value)>
inline auto to_nonnegative_int(T value, Int upper) -> Int {
  if (!std::is_unsigned<Int>::value &&
      (value < 0 || to_unsigned(value) > to_unsigned(upper))) {
    XTEXT_FMT_THROW(format_error("chrono value is out of range"));
  }
...
inline auto to_nonnegative_int(T value, Int upper) -> Int {
  ...
  if (int_value < 0 || value > static_cast<T>(upper))
    XTEXT_FMT_THROW(format_error("invalid value"));
  return int_value;
}
...
void write_fractional_seconds(OutputIt& out, Duration d, int precision = -1) {
  ...
  if (precision < 0) {
    XTEXT_FMT_ASSERT(!std::is_floating_point<typename Duration::rep>::value, "");
...
void write_floating_seconds(memory_buffer& buf, Duration duration,
  ...
  XTEXT_FMT_ASSERT(std::is_floating_point<rep>::value, "");
  ...
  fmt::format_to(std::back_inserter(buf), XTEXT_FMT_STRING("{:.{}f}"),
...
class tm_writer {
 private:
  ...
  auto tm_sec() const noexcept -> int {
    XTEXT_FMT_ASSERT(tm_.tm_sec >= 0 && tm_.tm_sec <= 61, "");
    return tm_.tm_sec;
  }
  auto tm_min() const noexcept -> int {
    XTEXT_FMT_ASSERT(tm_.tm_min >= 0 && tm_.tm_min <= 59, "");
    return tm_.tm_min;
  }
  auto tm_hour() const noexcept -> int {
    XTEXT_FMT_ASSERT(tm_.tm_hour >= 0 && tm_.tm_hour <= 23, "");
    return tm_.tm_hour;
  }
  auto tm_mday() const noexcept -> int {
    XTEXT_FMT_ASSERT(tm_.tm_mday >= 1 && tm_.tm_mday <= 31, "");
    return tm_.tm_mday;
  }
  auto tm_mon() const noexcept -> int {
    XTEXT_FMT_ASSERT(tm_.tm_mon >= 0 && tm_.tm_mon <= 11, "");
    return tm_.tm_mon;
  }
  auto tm_year() const noexcept -> long long { return 1900ll + tm_.tm_year; }
  auto tm_wday() const noexcept -> int {
    XTEXT_FMT_ASSERT(tm_.tm_wday >= 0 && tm_.tm_wday <= 6, "");
    return tm_.tm_wday;
  }
  auto tm_yday() const noexcept -> int {
    XTEXT_FMT_ASSERT(tm_.tm_yday >= 0 && tm_.tm_yday <= 365, "");
    return tm_.tm_yday;
  }
  ...
  template <typename T, XTEXT_FMT_ENABLE_IF(has_tm_gmtoff<T>::value)>
 ...
 public:
  ...
  XTEXT_FMT_CONSTEXPR void on_text(const Char* begin, const Char* end) {
...
struct chrono_format_checker : null_chrono_spec_handler<chrono_format_checker> {
  ...
  XTEXT_FMT_NORETURN inline void unsupported() { XTEXT_FMT_THROW(format_error("no date")); }
  ...
  XTEXT_FMT_CONSTEXPR void on_iso_time() {}
  XTEXT_FMT_CONSTEXPR void on_am_pm() {}
  XTEXT_FMT_CONSTEXPR void on_duration_value() const {
    if (has_precision_integral)
      XTEXT_FMT_THROW(format_error("precision not allowed for this argument type"));
  }
  XTEXT_FMT_CONSTEXPR void on_duration_unit() {}
};

template <typename T,
          XTEXT_FMT_ENABLE_IF(std::is_integral<T>::value&& has_isfinite<T>::value)>
...
template <typename T, XTEXT_FMT_ENABLE_IF(std::is_floating_point<T>::value)>
inline auto mod(T x, int y) -> T {
...
template <typename Rep, typename Period,
          XTEXT_FMT_ENABLE_IF(std::is_integral<Rep>::value)>
...
template <typename Char, typename Rep, typename OutputIt,
          XTEXT_FMT_ENABLE_IF(std::is_floating_point<Rep>::value)>
...
template <typename Char> struct formatter<std::tm, Char> {
 ...
 protected:
  ...
  XTEXT_FMT_CONSTEXPR void set_localized() { specs_.set_localized(); }

  XTEXT_FMT_CONSTEXPR auto do_parse(parse_context<Char>& ctx, bool has_timezone)
      -> const Char* {
    auto it = ctx.begin(), end = ctx.end();
    if (it == end || *it == '}') return it;
