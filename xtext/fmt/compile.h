...
#ifndef XTEXT_FMT_COMPILE_H_
#define XTEXT_FMT_COMPILE_H_
 ...
 *     std::string s = fmt::format(XTEXT_FMT_COMPILE("{}"), 42);
 */
#if defined(__cpp_if_constexpr) && defined(__cpp_return_type_deduction)
#  define XTEXT_FMT_COMPILE(s) XTEXT_FMT_STRING_IMPL(s, fmt::compiled_string)
#else
#  define XTEXT_FMT_COMPILE(s) XTEXT_FMT_STRING(s)
#endif
...
template <detail::fixed_string Str> constexpr auto operator""_cf() {
  return XTEXT_FMT_COMPILE(Str.data);
}
}  // namespace literals
#endif
...
XTEXT_FMT_CONSTEXPR auto get_arg_index_by_name(basic_string_view<Char> name) -> int {
#  if XTEXT_FMT_USE_NONTYPE_TEMPLATE_ARGS
  if constexpr (sizeof...(Args) > 0)
...
template <typename Char> struct runtime_named_field {
  ...
  constexpr auto format(OutputIt out, const T&... args) const -> OutputIt {
    ...
    if (!found) {
      XTEXT_FMT_THROW(format_error("argument with specified name is not found"));
    }
    return out;
  }
};
...
template <typename Char, typename V, int N> struct spec_field {
  ...
  template <typename OutputIt, typename... T>
  constexpr XTEXT_FMT_INLINE auto format(OutputIt out, const T&... args) const
      -> OutputIt {
    const auto& vargs =
...
template <typename Char> struct arg_id_handler {
  ...
  constexpr auto on_auto() -> int {
    XTEXT_FMT_ASSERT(false, "handler cannot be used with automatic indexing");
    return 0;
  }
...
constexpr auto parse_replacement_field_then_tail(S fmt) {
  ...
  if constexpr (c == '}') {
    ...
    XTEXT_FMT_THROW(format_error("expected ':'"));
  } else {
...
constexpr auto compile_format_string(S fmt) {
  ...
  if constexpr (str[POS] == '{') {
    if constexpr (POS + 1 == str.size())
      XTEXT_FMT_THROW(format_error("unmatched '{' in format string"));
...
template <typename... Args, typename S,
          XTEXT_FMT_ENABLE_IF(is_compiled_string<S>::value)>
...
template <typename CompiledFormat, typename... T,
          ...
          XTEXT_FMT_ENABLE_IF(detail::is_compiled_format<CompiledFormat>::value)>
XTEXT_FMT_INLINE XTEXT_FMT_CONSTEXPR_STRING auto format(const CompiledFormat& cf,
                                            const T&... args)
...
template <typename OutputIt, typename CompiledFormat, typename... T,
          XTEXT_FMT_ENABLE_IF(detail::is_compiled_format<CompiledFormat>::value)>
constexpr XTEXT_FMT_INLINE auto format_to(OutputIt out, const CompiledFormat& cf,
...
template <typename S, typename... T,
          XTEXT_FMT_ENABLE_IF(is_compiled_string<S>::value)>
XTEXT_FMT_INLINE XTEXT_FMT_CONSTEXPR_STRING auto format(const S&, T&&... args)
...
template <typename OutputIt, typename S, typename... T,
          XTEXT_FMT_ENABLE_IF(is_compiled_string<S>::value)>
XTEXT_FMT_CONSTEXPR auto format_to(OutputIt out, const S&, T&&... args) -> OutputIt {
  constexpr auto compiled = detail::compile<T...>(S());
  if constexpr (std::is_same<remove_cvref_t<decltype(compiled)>,
                             detail::unknown_format>()) {
...
template <typename S, typename... T,
          XTEXT_FMT_ENABLE_IF(is_compiled_string<S>::value)>
XTEXT_FMT_CONSTEXPR20 auto formatted_size(const S& fmt, T&&... args) -> size_t {
...
template <size_t N> class static_format_result {
 ...
 public:
  template <typename S, typename... T,
            XTEXT_FMT_ENABLE_IF(is_compiled_string<S>::value)>
  explicit XTEXT_FMT_CONSTEXPR static_format_result(const S& fmt, T&&... args) {
 ...
 *     static constexpr auto result = XTEXT_FMT_STATIC_FORMAT("{}", 42);
 *     const char* s = result.c_str();
 */
#define XTEXT_FMT_STATIC_FORMAT(fmt_str, ...)                            \
  fmt::static_format_result<                                       \
      fmt::formatted_size(XTEXT_FMT_COMPILE(fmt_str), __VA_ARGS__) + 1>( \
      XTEXT_FMT_COMPILE(fmt_str), __VA_ARGS__)

XTEXT_FMT_END_EXPORT
XTEXT_FMT_END_NAMESPACE

#endif  // XTEXT_FMT_COMPILE_H_
