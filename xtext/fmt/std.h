...
#ifndef XTEXT_FMT_STD_H_
#define XTEXT_FMT_STD_H_
...
// Check XTEXT_FMT_CPLUSPLUS to suppress a bogus warning in MSVC.
#  if XTEXT_FMT_CPLUSPLUS >= 201703L
#    if XTEXT_FMT_HAS_INCLUDE(<filesystem>) && \
        (!defined(XTEXT_FMT_CPP_LIB_FILESYSTEM) || XTEXT_FMT_CPP_LIB_FILESYSTEM != 0)
#      include <filesystem>
#    endif
#    if XTEXT_FMT_HAS_INCLUDE(<variant>)
#      include <variant>
#    endif
#    if XTEXT_FMT_HAS_INCLUDE(<optional>)
#      include <optional>
#    endif
#  endif
// Use > instead of >= in the version check because <source_location> may be
// available after C++17 but before C++20 is marked as implemented.
#  if XTEXT_FMT_CPLUSPLUS > 201703L && XTEXT_FMT_HAS_INCLUDE(<source_location>)
#    include <source_location>
#  endif
#  if XTEXT_FMT_CPLUSPLUS > 202002L && XTEXT_FMT_HAS_INCLUDE(<expected>)
#    include <expected>
#  endif
#endif  // XTEXT_FMT_MODULE

#if XTEXT_FMT_HAS_INCLUDE(<version>)
#  include <version>
#endif

// GCC 4 does not support XTEXT_FMT_HAS_INCLUDE.
#if XTEXT_FMT_HAS_INCLUDE(<cxxabi.h>) || defined(__GLIBCXX__)
#  include <cxxabi.h>
// Android NDK with gabi++ library on some architectures does not implement
// abi::__cxa_demangle().
#  ifndef __GABIXX_CXXABI_H__
#    define XTEXT_FMT_HAS_ABI_CXA_DEMANGLE
#  endif
#endif

#ifdef XTEXT_FMT_CPP_LIB_FILESYSTEM
// Use the provided definition.
#elif defined(__cpp_lib_filesystem)
#  define XTEXT_FMT_CPP_LIB_FILESYSTEM __cpp_lib_filesystem
#else
#  define XTEXT_FMT_CPP_LIB_FILESYSTEM 0
#endif

#ifdef XTEXT_FMT_CPP_LIB_VARIANT
// Use the provided definition.
#elif defined(__cpp_lib_variant)
#  define XTEXT_FMT_CPP_LIB_VARIANT __cpp_lib_variant
#else
#  define XTEXT_FMT_CPP_LIB_VARIANT 0
#endif

XTEXT_FMT_BEGIN_NAMESPACE
namespace detail {

#if XTEXT_FMT_CPP_LIB_FILESYSTEM
...
void write_escaped_path(basic_memory_buffer<Char>& quoted,
  ...
  if constexpr (std::is_same_v<Char, char> &&
    ...
    XTEXT_FMT_ASSERT(valid, "invalid utf16");
...
#endif  // XTEXT_FMT_CPP_LIB_FILESYSTEM

#if defined(__cpp_lib_expected) || XTEXT_FMT_CPP_LIB_VARIANT
...
#endif  // XTEXT_FMT_CPP_LIB_VARIANT

#if XTEXT_FMT_USE_RTTI
...
#  ifdef XTEXT_FMT_HAS_ABI_CXA_DEMANGLE
  int status = 0;
  size_t size = 0;
...
#  elif XTEXT_FMT_MSC_VERSION && defined(_MSVC_STL_UPDATE)
  return normalize_msvc_abi_name(ti.name(), out);
#  elif XTEXT_FMT_MSC_VERSION && defined(_LIBCPP_VERSION)
...
#if defined(_LIBCPP_VERSION) && !defined(XTEXT_FMT_IMPORT_STD)
template <typename C>
...
template <typename Char> struct formatter<std::filesystem::path, Char> {
 ...
 public:
  XTEXT_FMT_CONSTEXPR void set_debug_format(bool set = true) { debug_ = set; }

  XTEXT_FMT_CONSTEXPR auto parse(parse_context<Char>& ctx) {
...
class path : public std::filesystem::path {
 public:
  auto display_string() const -> std::string {
    ...
    return fmt::format(XTEXT_FMT_STRING("{}"), base);
  }
  auto system_string() const -> std::string { return string(); }

  auto generic_display_string() const -> std::string {
    const std::filesystem::path& base = *this;
    return fmt::format(XTEXT_FMT_STRING("{:g}"), base);
  }
...
struct formatter<std::bitset<N>, Char>
 ...
 private:
  ...
  struct writer {
    ...
    template <typename OutputIt>
    XTEXT_FMT_CONSTEXPR auto operator()(OutputIt out) -> OutputIt {
...
template <> struct formatter<std::source_location> {
  XTEXT_FMT_CONSTEXPR auto parse(parse_context<>& ctx) { return ctx.begin(); }
...
struct formatter<Variant, Char,
  ...
  auto format(const Variant& value, FormatContext& ctx) const
    ...
    XTEXT_FMT_CATCH(const std::bad_variant_access&) {
...
template <> struct formatter<std::error_code> {
 ...
 public:
  ...
  template <typename FormatContext>
  XTEXT_FMT_CONSTEXPR20 auto format(const std::error_code& ec,
...
struct formatter<
 ...
 public:
  XTEXT_FMT_CONSTEXPR auto parse(parse_context<>& ctx) -> const char* {
    ...
    if (*it == 't') {
      ++it;
      with_typename_ = XTEXT_FMT_USE_RTTI != 0;
    }
    return it;
  }
...
struct formatter<BitRef, Char,
    ...
    : formatter<bool, Char> {
  template <typename FormatContext>
  XTEXT_FMT_CONSTEXPR auto format(const BitRef& v, FormatContext& ctx) const
      -> decltype(ctx.out()) {
...
template <typename T, typename Char> struct formatter<std::complex<T>, Char> {
 private:
  ...
  XTEXT_FMT_CONSTEXPR auto do_format(const std::complex<T>& c,
