#ifndef XTEXT_SIMDUTF_ERROR_H
#define XTEXT_SIMDUTF_ERROR_H
#include <string_view>
...
struct result {
  ...
  xtext_really_inline xtext_constexpr23 result() noexcept
      : error{error_code::SUCCESS}, count{0} {}

  xtext_really_inline xtext_constexpr23 result(error_code err,
                                                   size_t pos) noexcept
      : error{err}, count{pos} {}

  xtext_really_inline xtext_constexpr23 bool is_ok() const noexcept {
    return error == error_code::SUCCESS;
  }

  xtext_really_inline xtext_constexpr23 bool is_err() const noexcept {
    return error != error_code::SUCCESS;
  }
};
...
struct full_result {
  ...
  xtext_really_inline xtext_constexpr23 full_result() noexcept
      : error{error_code::SUCCESS}, input_count{0}, output_count{0} {}

  xtext_really_inline xtext_constexpr23 full_result(error_code err,
                                                        size_t pos_in,
                                                        size_t pos_out) noexcept
      : error{err}, input_count{pos_in}, output_count{pos_out} {}
  xtext_really_inline xtext_constexpr23 full_result(
      error_code err, size_t pos_in, size_t pos_out, bool padding_err) noexcept
      : error{err}, input_count{pos_in}, output_count{pos_out},
        padding_error{padding_err} {}

  xtext_really_inline xtext_constexpr23 operator result() const noexcept {
