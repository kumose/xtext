...
//
xtext_really_inline simd8<uint8_t> is_incomplete(const simd8<uint8_t> input) {
...
struct utf8_checker {
  ...
  //
  xtext_really_inline void check_utf8_bytes(const simd8<uint8_t> input,
                                              const simd8<uint8_t> prev_input) {
  ...
  xtext_really_inline void check_next_input(const simd8x64<uint8_t> &input) {
    if (xtext_likely(is_ascii(input))) {
      this->error |= this->prev_incomplete;
    } else {
