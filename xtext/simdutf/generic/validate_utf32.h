...
xtext_really_inline bool validate(const char32_t *input, size_t size) {
  if (xtext_unlikely(size == 0)) {
...
xtext_really_inline result validate_with_errors(const char32_t *input,
                                                  size_t size) {
  if (xtext_unlikely(size == 0)) {
    // empty input is valid UTF-32. protect the implementation from
    // handling nullptr
  ...
  while (input + N < end) {
    ...
    if (xtext_unlikely(combined.any())) {
