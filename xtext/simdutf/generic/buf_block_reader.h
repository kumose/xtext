...
public:
  xtext_really_inline buf_block_reader(const uint8_t *_buf, size_t _len);
  xtext_really_inline size_t block_index();
  xtext_really_inline bool has_full_block() const;
  xtext_really_inline const uint8_t *full_block() const;
  /**
   ...
   */
  xtext_really_inline size_t get_remainder(uint8_t *dst) const;
  xtext_really_inline void advance();
...
template <size_t STEP_SIZE>
xtext_really_inline size_t buf_block_reader<STEP_SIZE>::block_index() {
  return idx;
}

template <size_t STEP_SIZE>
xtext_really_inline bool buf_block_reader<STEP_SIZE>::has_full_block() const {
  return idx < lenminusstep;
}

template <size_t STEP_SIZE>
xtext_really_inline const uint8_t *
...
template <size_t STEP_SIZE>
xtext_really_inline void buf_block_reader<STEP_SIZE>::advance() {
  idx += STEP_SIZE;
}
