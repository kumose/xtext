#ifndef XTEXT_SIMDUTF_UTF8_H
#define XTEXT_SIMDUTF_UTF8_H
...
xtext_constexpr23 xtext_warn_unused bool validate(BytePtr data,
                                                      size_t len) noexcept {
  static_assert(
...
xtext_really_inline xtext_warn_unused bool validate(const char *buf,
                                                        size_t len) noexcept {
  return validate(reinterpret_cast<const uint8_t *>(buf), len);
}

template <class BytePtr>
xtext_constexpr23 xtext_warn_unused result
...
xtext_really_inline xtext_warn_unused result
...
inline xtext_warn_unused result rewind_and_validate_with_errors(
...
template <typename InputPtr>
  ...
  requires xtext::detail::indexes_into_byte_like<InputPtr>
#endif
xtext_constexpr23 size_t count_code_points(InputPtr data, size_t len) {
  size_t counter{0};
...
template <typename InputPtr>
  ...
  requires xtext::detail::indexes_into_byte_like<InputPtr>
#endif
xtext_constexpr23 size_t utf16_length_from_utf8(InputPtr data, size_t len) {
  size_t counter{0};
  for (size_t i = 0; i < len; i++) {
    if (int8_t(data[i]) > -65) {
      counter++;
