#ifndef XTEXT_SIMDUTF_BASE64_H
#define XTEXT_SIMDUTF_BASE64_H
...
template <class char_type> xtext_constexpr23 bool is_eight_byte(char_type c) {
...
xtext_constexpr23 bool is_ignorable(char_type c,
...
template <class char_type>
xtext_constexpr23 bool is_base64(char_type c,
                                   xtext::base64_options options) {
  const uint8_t *to_base64 =
      (options & base64_default_or_url)
          ? tables::base64::to_base64_default_or_url_value
...
template <class char_type>
xtext_constexpr23 bool is_base64_or_padding(char_type c,
                                              xtext::base64_options options) {
  const uint8_t *to_base64 =
      (options & base64_default_or_url)
          ? tables::base64::to_base64_default_or_url_value
...
template <class char_type>
bool is_ignorable_or_padding(char_type c, xtext::base64_options options) {
...
template <class char_type>
xtext_constexpr23 reduced_input find_end(const char_type *src, size_t srclen,
                                           xtext::base64_options options) {
  const uint8_t *to_base64 =
      (options & base64_default_or_url)
          ? tables::base64::to_base64_default_or_url_value
  ...
  if (ignore_garbage) {
    ...
    auto it = xtext::find(src, src + srclen, '=');
    if (it != src + srclen) {
...
xtext_constexpr23 full_result base64_tail_decode_impl(
  ...
  while (true) {
    ...
    if (idx != 4) {
      xtext_log_assert(idx < 4, "idx should be less than 4");
...
template <class char_type>
xtext_constexpr23 full_result base64_tail_decode(
...
//
template <class char_type>
xtext_constexpr23 full_result base64_tail_decode_safe(
    char *dst, size_t outlen, const char_type *src, size_t length,
    size_t padding_characters, // number of padding characters
                               // '=', typically 0, 1, 2.
    base64_options options, last_chunk_handling_options last_chunk_options) {
...
inline xtext_constexpr23 full_result
...
xtext_constexpr23 size_t tail_encode_base64_impl(
    char *dst, const char *src, size_t srclen, base64_options options,
    size_t line_length = xtext::default_line_length, size_t line_offset = 0) {
  if constexpr (use_lines) {
    ...
    xtext_log_assert(line_offset <= line_length,
...
// Returns the number of bytes written. The destination buffer must be large
// enough. It will add padding (=) if needed.
xtext_unused inline xtext_constexpr23 size_t tail_encode_base64(
    char *dst, const char *src, size_t srclen, base64_options options) {
  return tail_encode_base64_impl(dst, src, srclen, options);
}

template <class InputPtr>
xtext_warn_unused xtext_constexpr23 size_t
...
xtext_warn_unused xtext_constexpr23 full_result
base64_to_binary_details_impl(
  ...
  const bool ignore_garbage =
      ...
      (options == base64_options::base64_default_accept_garbage) ||
      (options == base64_options::base64_default_or_url_accept_garbage);
  auto ri = xtext::scalar::base64::find_end(input, length, options);
...
template <typename char_type>
xtext_constexpr23 xtext_warn_unused full_result
...
// the input.
template <typename char_type>
xtext_warn_unused size_t prefix_length(size_t count,
                                         xtext::base64_options options,
  ...
  xtext_log_assert(false, "You never get here");
