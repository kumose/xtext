#ifndef XTEXT_SIMDUTF_SWAP_BYTES_H
#define XTEXT_SIMDUTF_SWAP_BYTES_H

namespace xtext {
namespace scalar {

constexpr inline xtext_warn_unused uint16_t
u16_swap_bytes(const uint16_t word) {
  return uint16_t((word >> 8) | (word << 8));
}

constexpr inline xtext_warn_unused uint32_t
