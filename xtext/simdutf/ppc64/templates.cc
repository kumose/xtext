...
size_t convert_impl(VectorizedConvert vectorized_convert,
  ...
  if (vr.err != xtext::error_code::SUCCESS) {
    if (vr.err == xtext::error_code::OTHER) {
...
template <typename VectorizedConvert, typename ScalarConvert, typename Source,
          typename Destination>
xtext::result convert_with_errors_impl(VectorizedConvert vectorized_convert,
  ...
  if (vr.err != xtext::error_code::SUCCESS) {
    ...
    return xtext::result(vr.err, consumed);
  }

  if (consumed == len) {
    return xtext::result(xtext::error_code::SUCCESS, written);
  }

  xtext::result sr = scalar_convert(vr.input, len - consumed, vr.output);
  if (sr.is_ok()) {
    sr.count += written;
  } else {
    sr.count += consumed;
