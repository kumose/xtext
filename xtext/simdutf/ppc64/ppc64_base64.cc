...
xtext_really_inline vector_u8 encoding_expand_6bit_fields(vector_u8 input) {
...
static xtext_really_inline void compress(const vector_u8 data, uint16_t mask,
                                           char *output) {
  if (mask == 0) {
    data.store(output);
    return;
...
static xtext_really_inline vector_u8 decoding_pack(vector_u8 input) {
#if XTEXT_SIMDUTF_IS_BIG_ENDIAN
...
static xtext_really_inline void base64_decode(char *out, vector_u8 input) {
  const auto expanded = decoding_pack(input);
  expanded.store(out);
}

static xtext_really_inline void base64_decode_block(char *out,
                                                      const char *src) {
...
static xtext_really_inline void base64_decode_block_safe(char *out,
                                                           const char *src) {
  base64_decode(out + 12 * 0, vector_u8::load(src + 0 * 16));
  base64_decode(out + 12 * 1, vector_u8::load(src + 1 * 16));
  base64_decode(out + 12 * 2, vector_u8::load(src + 2 * 16));
...
public:
  xtext_really_inline block64(const char *src) : b(load_block(src)) {}
  xtext_really_inline block64(const char16_t *src) : b(load_block(src)) {}
...
private:
  ...
  // structure.
  static xtext_really_inline simd8x64<uint8_t> load_block(const char *src) {
  ...
  // bytes available from reading at src. The data is read into a block64
  // structure.
  static xtext_really_inline simd8x64<uint8_t>
