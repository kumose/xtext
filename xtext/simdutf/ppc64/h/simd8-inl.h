...
template <typename T> struct base8 {
  ...
  // Zero constructor
  xtext_really_inline base8() : value{vec_splats(T(0))} {}

  // Conversion from SIMD register
  xtext_really_inline base8(const vector_type _value) : value{_value} {}

  // Splat scalar
  xtext_really_inline base8(T v) : value{vec_splats(v)} {}

  // Conversion to SIMD register
  xtext_really_inline operator const vector_type &() const {
    return this->value;
  }

  template <typename U> xtext_really_inline void store(U *ptr) const {
  ...
  xtext_really_inline void store_bytes_as_utf16(char16_t *p) const {
  ...
  xtext_really_inline void store_bytes_as_utf32(char32_t *p) const {
    const vector_type zero = vec_splats(T(0));
...
#endif // XTEXT_SIMDUTF_IS_BIG_ENDIAN
  ...
  xtext_really_inline void store_words_as_utf32(char32_t *p) const {
    const vector_type zero = vec_splats(T(0));
...
template <> struct simd8<bool> : base8<bool> {
  ...
  xtext_really_inline simd8() : super(vector_type()) {}
  xtext_really_inline simd8(const vector_type _value) : super(_value) {}
  // Splat constructor
  xtext_really_inline simd8(bool _value) : base8<bool>(splat(_value)) {}

  template <typename T>
  xtext_really_inline simd8(simd8<T> other)
  ...
  xtext_really_inline bool all() const { return to_bitmask() == 0xffff; }

  xtext_really_inline simd8<bool> operator~() const {
...
template <typename T> struct base8_numeric : base8<T> {
  ...
  static xtext_really_inline simd8<T> splat(T value) {
    return (vector_type)vec_splats(value);
  }

  static xtext_really_inline simd8<T> zero() { return splat(0); }

  template <typename U>
  static xtext_really_inline simd8<T> load(const U *values) {
  ...
  xtext_really_inline base8_numeric(const vector_type _value)
      : base8<T>(_value) {}
...
template <> struct simd8<uint8_t> : base8_numeric<uint8_t> {
  ...
  xtext_really_inline simd8(const vector_type _value)
  ...
  template <int N>
  xtext_really_inline Self prev(const Self prev_chunk) const {
...
xtext_really_inline simd8<uint8_t> avg(const simd8<uint8_t> a,
