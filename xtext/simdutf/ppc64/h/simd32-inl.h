...
template <typename T> struct base32 {
  ...
  // Zero constructor
  xtext_really_inline base32() : value{vector_type()} {}

  // Conversion from SIMD register
  xtext_really_inline base32(const vector_type _value) : value{_value} {}

  // Splat for scalar
  xtext_really_inline base32(T scalar) : value{vec_splats(scalar)} {}

  template <typename Pointer>
  xtext_really_inline base32(const Pointer *ptr)
...
template <typename T> struct base32_numeric : base32<T> {
  ...
  static xtext_really_inline simd32<T> splat(T _value) {
    return vec_splats(_value);
  }

  static xtext_really_inline simd32<T> zero() { return splat(0); }

  template <typename U>
  static xtext_really_inline simd32<T> load(const U *values) {
    return vec_xl(0, reinterpret_cast<const T *>(values));
  }

  xtext_really_inline base32_numeric() : base32<T>() {}

  xtext_really_inline base32_numeric(const vector_type _value)
      : base32<T>(_value) {}

  // Addition/subtraction are the same for signed and unsigned
  xtext_really_inline simd32<T> operator+(const simd32<T> other) const {
  ...
  xtext_really_inline simd32<T> &operator+=(const simd32<T> other) {
    *this = *this + other;
...
template <> struct simd32<bool> : base32<bool> {
  static xtext_really_inline simd32<bool> splat(bool _value) {
    return (vector_type)vec_splats(uint32_t(-(!!_value)));
  }

  xtext_really_inline simd32(const vector_type _value)
      : base32<bool>(_value) {}

  // Splat constructor
  xtext_really_inline simd32(bool _value) : base32<bool>(splat(_value)) {}
  ...
  xtext_really_inline simd32<bool> operator~() const {
...
template <> struct simd32<uint32_t> : base32_numeric<uint32_t> {
  xtext_really_inline simd32() : base32_numeric<uint32_t>() {}

  xtext_really_inline simd32(const vector_type _value)
      : base32_numeric<uint32_t>(_value) {}

  // Splat constructor
  xtext_really_inline simd32(uint32_t _value) : simd32(splat(_value)) {}

  // Array constructor
  xtext_really_inline simd32(const char32_t *values)
      : simd32(load(reinterpret_cast<const uint32_t *>(values))) {}

  // Bit-specific operations
  template <int N> xtext_really_inline simd32<uint32_t> shr() const {
    return vec_sr(value, vec_splats(uint32_t(N)));
  }

  template <int N> xtext_really_inline simd32<uint32_t> shl() const {
    return vec_sl(value, vec_splats(uint32_t(N)));
  }

  // Change the endianness
  xtext_really_inline simd32<uint32_t> swap_bytes() const {
  ...
  static xtext_really_inline simd16<uint16_t>
...
template <typename T>
xtext_really_inline simd32<T> min(const simd32<T> b, const simd32<T> a) {
