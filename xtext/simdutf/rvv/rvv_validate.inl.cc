...
xtext_really_inline bool rvv_validate_utf16_as_ascii(const char16_t *buf,
                                                       size_t len) noexcept {
...
implementation::validate_utf16le_as_ascii(const char16_t *buf,
  ...
  return rvv_validate_utf16_as_ascii<xtext_ByteFlip::NONE>(buf, len);
}
...
implementation::validate_utf16be_as_ascii(const char16_t *buf,
                                          size_t len) const noexcept {
  if (supports_zvbb())
    return rvv_validate_utf16_as_ascii<xtext_ByteFlip::ZVBB>(buf, len);
  else
    return rvv_validate_utf16_as_ascii<xtext_ByteFlip::V>(buf, len);
}
...
xtext_really_inline static size_t rvv_count_valid_utf8(const char *src,
                                                         size_t len) {
  const char *beg = src;
  if (len < 32)
    return 0;
  ...
  for (size_t vl; n > 0; n -= vl, src += vl) {
    ...
    vuint8m4_t err1 = xtext_vrgather_u8m1x4(err1tbl, idx1);
    vuint8m4_t err2 = xtext_vrgather_u8m1x4(err2tbl, idx2);
    vuint8m4_t err3 = xtext_vrgather_u8m1x4(err3tbl, idx3);
...
rvv_validate_utf16_with_errors(const char16_t *src, size_t len) {
  ...
  const uint16_t mask = xtext_byteflip<bflip>(0xfc00);
  const uint16_t hi_surrogate = xtext_byteflip<bflip>(0xd800);
  const uint16_t lo_surrogate = xtext_byteflip<bflip>(0xdc00);
  ...
  for (size_t vl; len > 0; len -= vl, src += vl, last = src[-1]) {
    ...
    if (idx >= 0) {
      last = xtext_byteflip<bflip>(idx > 0 ? src[idx - 1] : last);
      return result(error_code::SURROGATE,
                    src - beg + idx - (last - 0xD800u < 0x400u));
      break;
    }
  }
  if (xtext_byteflip<bflip>(last) - 0xD800u < 0x400u) {
    return result(error_code::SURROGATE,
...
implementation::validate_utf16le(const char16_t *src,
                                 size_t len) const noexcept {
  return rvv_validate_utf16_with_errors<xtext_ByteFlip::NONE>(src, len)
...
xtext_warn_unused result implementation::validate_utf16be_with_errors(
    const char16_t *src, size_t len) const noexcept {
  if (supports_zvbb())
    return rvv_validate_utf16_with_errors<xtext_ByteFlip::ZVBB>(src, len);
  else
    return rvv_validate_utf16_with_errors<xtext_ByteFlip::V>(src, len);
}
#endif // XTEXT_SIMDUTF_FEATURE_UTF16
