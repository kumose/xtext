...
xtext_really_inline void utf16fix_block_rvv(char16_t *out, const char16_t *in,
                                              size_t vl) {
...
xtext_really_inline static void
rvv_change_endianness_utf16(const char16_t *src, size_t len, char16_t *dst) {
  for (size_t vl; len > 0; len -= vl, src += vl, dst += vl) {
    ...
    __riscv_vse16_v_u16m8((uint16_t *)dst, xtext_byteflip<bflip>(v, vl), vl);
  }
}
...
void implementation::change_endianness_utf16(const char16_t *src, size_t len,
  ...
  if (supports_zvbb())
    return rvv_change_endianness_utf16<xtext_ByteFlip::ZVBB>(src, len, dst);
  else
    return rvv_change_endianness_utf16<xtext_ByteFlip::V>(src, len, dst);
}
