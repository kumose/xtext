...
xtext_warn_unused result convert_utf32_to_utf8_aux(const char32_t *src,
                                                     size_t len,
                                                     char *dst) noexcept {
  size_t n = len;
...
rvv_convert_utf32_to_utf16_with_errors(const char32_t *src, size_t len,
  ...
  for (size_t vl, vlOut; len > 0; len -= vl, src += vl, dst += vlOut) {
    ...
    if (idx < 0) {
      ...
      vlOut = vl;
      vuint16m2_t n =
          xtext_byteflip<bflip>(__riscv_vncvt_x_x_w_u16m2(v, vlOut), vlOut);
...
xtext_warn_unused result implementation::convert_utf32_to_utf16le_with_errors(
    const char32_t *src, size_t len, char16_t *dst) const noexcept {
  return rvv_convert_utf32_to_utf16_with_errors<xtext_ByteFlip::NONE>(
      src, len, dst);
}
...
xtext_warn_unused result implementation::convert_utf32_to_utf16be_with_errors(
    const char32_t *src, size_t len, char16_t *dst) const noexcept {
  if (supports_zvbb())
    return rvv_convert_utf32_to_utf16_with_errors<xtext_ByteFlip::ZVBB>(
        src, len, dst);
  else
    return rvv_convert_utf32_to_utf16_with_errors<xtext_ByteFlip::V>(src, len,
                                                                       dst);
}

template <xtext_ByteFlip bflip>
xtext_really_inline static size_t
...
xtext_warn_unused size_t implementation::convert_valid_utf32_to_utf16le(
    const char32_t *src, size_t len, char16_t *dst) const noexcept {
  return rvv_convert_valid_utf32_to_utf16<xtext_ByteFlip::NONE>(src, len,
                                                                  dst);
}
...
xtext_warn_unused size_t implementation::convert_valid_utf32_to_utf16be(
    const char32_t *src, size_t len, char16_t *dst) const noexcept {
  if (supports_zvbb())
    return rvv_convert_valid_utf32_to_utf16<xtext_ByteFlip::ZVBB>(src, len,
                                                                    dst);
  else
    return rvv_convert_valid_utf32_to_utf16<xtext_ByteFlip::V>(src, len, dst);
}
