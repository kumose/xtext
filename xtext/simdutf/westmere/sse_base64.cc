...
static xtext_really_inline void compress(__m128i data, uint16_t mask,
                                           char *output) {
  if (mask == 0) {
...
static xtext_really_inline void base64_decode(char *out, __m128i str) {
  // credit: aqrit
...
private:
  template <bool base64_url, bool ignore_garbage, bool default_or_url>
  xtext_really_inline uint16_t to_base64_mask(__m128i *src, uint32_t *error) {
