...
template <typename T, typename Mask = simd16<bool>> struct base_u16 {
  ...
  xtext_really_inline base_u16(const __m128i _value) : value(_value) {}
  // Bit operations
  ...
  template <unsigned N>
  xtext_really_inline simd16<T> byte_right_shift() const {
    return __lsx_vbsrl_v(this->value, N);
  }

  xtext_really_inline uint16_t first() const {
...
struct base16 : base_u16<T> {
  ...
  xtext_really_inline base16(const __m128i _value) : base_u16<T>(_value) {}
  template <typename Pointer>
  xtext_really_inline base16(const Pointer *ptr)
...
template <> struct simd16<bool> : base16<bool> {
  ...
  xtext_really_inline simd16(const __m128i _value) : base16<bool>(_value) {}

  xtext_really_inline bitmask_type to_bitmask() const {
    __m128i mask = __lsx_vmsknz_b(this->value);
    bitmask_type mask0 = bitmask_type(__lsx_vpickve2gr_wu(mask, 0));
    return mask0;
  }

  xtext_really_inline bool is_zero() const { return __lsx_bz_v(this->value); }
};
...
template <typename T> struct base16_numeric : base16<T> {
  ...
  static xtext_really_inline simd16<T> zero() { return __lsx_vldi(0); }

  template <typename Pointer>
  static xtext_really_inline simd16<T> load(const Pointer values) {
    return __lsx_vld(values, 0);
  }

  xtext_really_inline base16_numeric(const __m128i _value)
      : base16<T>(_value) {}
...
template <> struct simd16<uint16_t> : base16_numeric<uint16_t> {
  xtext_really_inline simd16(const __m128i _value)
  ...
  // Copy constructor
  xtext_really_inline simd16(const simd16<bool> mask) : simd16(mask.value) {}

  // Order-specific operations
  xtext_really_inline simd16 &operator+=(const simd16 other) {
    value = __lsx_vadd_h(value, other.value);
    return *this;
  }

  template <unsigned N>
  static xtext_really_inline simd8<uint8_t>
...
xtext_really_inline simd16<bool> operator<(const simd16<uint16_t> a,
...
xtext_really_inline simd16<uint16_t> operator^(const simd16<uint16_t> a,
                                                 uint16_t b) {
  const auto bv = __lsx_vreplgr2vr_h(b);
  return __lsx_vxor_v(a.value, bv);
}

xtext_really_inline simd16<bool> operator^(const simd16<bool> a,
                                             const simd16<bool> b) {
...
xtext_really_inline simd16<uint16_t> as_vector_u16(const simd16<bool> x) {
  return x.value;
}
