...
template <> struct simd32<uint32_t> {
  ...
  xtext_really_inline simd32(const uint32x4_t v) : value(v) {}

  template <typename Pointer>
  xtext_really_inline simd32(const Pointer *ptr)
      : value(vld1q_u32(reinterpret_cast<const uint32_t *>(ptr))) {}

  xtext_really_inline uint64_t sum() const { return vaddvq_u32(value); }
  ...
  // static members
  xtext_really_inline static simd32<uint32_t> zero() {
    return vdupq_n_u32(0);
  }

  xtext_really_inline static simd32<uint32_t> splat(uint32_t v) {
...
template <> struct simd32<bool> {
  ...
  xtext_really_inline bool any() const { return vmaxvq_u32(value) != 0; }
};

//----------------------------------------------------------------------

template <typename T>
xtext_really_inline simd32<T> operator|(const simd32<T> a,
                                          const simd32<T> b) {
  return vorrq_u32(a.value, b.value);
}

xtext_really_inline simd32<uint32_t> min(const simd32<uint32_t> a,
                                           const simd32<uint32_t> b) {
  return vminq_u32(a.value, b.value);
}

xtext_really_inline simd32<uint32_t> max(const simd32<uint32_t> a,
                                           const simd32<uint32_t> b) {
...
xtext_really_inline simd32<bool> operator!(const simd32<bool> v) {
...
xtext_really_inline simd32<uint32_t> select(const simd32<bool> cond,
