...
template <typename T, typename Mask = simd16<bool>> struct base_u16 {
  ...
  xtext_really_inline base_u16() = default;
  xtext_really_inline base_u16(const uint16x8_t _value) : value(_value) {}
  xtext_really_inline operator const uint16x8_t &() const {
    return this->value;
  }
  xtext_really_inline operator uint16x8_t &() { return this->value; }
  // Bit operations
  xtext_really_inline simd16<T> operator|(const simd16<T> other) const {
  ...
  xtext_really_inline simd16<T> bit_andnot(const simd16<T> other) const {
    return vbicq_u16(*this, other);
  }
  xtext_really_inline simd16<T> operator~() const { return *this ^ 0xFFu; }
  xtext_really_inline simd16<T> &operator|=(const simd16<T> other) {
  ...
  friend xtext_really_inline Mask operator==(const simd16<T> lhs,
                                               const simd16<T> rhs) {
    return vceqq_u16(lhs, rhs);
  }

  template <int N = 1>
  xtext_really_inline simd16<T> prev(const simd16<T> prev_chunk) const {
...
struct base16 : base_u16<T> {
  ...
  xtext_really_inline base16() : base_u16<T>() {}
  xtext_really_inline base16(const uint16x8_t _value) : base_u16<T>(_value) {}
  template <typename Pointer>
  xtext_really_inline base16(const Pointer *ptr) : base16(vld1q_u16(ptr)) {}
...
template <> struct simd16<bool> : base16<bool> {
  ...
  xtext_really_inline simd16(const uint16x8_t _value)
      : base16<bool>(_value) {}
  // Splat constructor
...
template <typename T> struct base16_numeric : base16<T> {
  ...
  static xtext_really_inline simd16<T> zero() { return vdupq_n_u16(0); }
  static xtext_really_inline simd16<T> load(const T values[8]) {
    return vld1q_u16(reinterpret_cast<const uint16_t *>(values));
  }

  xtext_really_inline base16_numeric() : base16<T>() {}
  xtext_really_inline base16_numeric(const uint16x8_t _value)
      : base16<T>(_value) {}

  // Store to array
  xtext_really_inline void store(T dst[8]) const {
...
template <> struct simd16<int16_t> : base16_numeric<int16_t> {
...
#endif
  xtext_really_inline simd16(const int16x8_t _value)
      : base16_numeric<int16_t>(vreinterpretq_u16_s16(_value)) {}

  // Splat constructor
  xtext_really_inline simd16(int16_t _value) : simd16(splat(_value)) {}
  // Array constructor
  xtext_really_inline simd16(const int16_t *values) : simd16(load(values)) {}
  ...
  xtext_really_inline operator const int16x8_t() const {
    return vreinterpretq_s16_u16(this->value);
  }

  xtext_really_inline int16_t max_val() const {
    return vmaxvq_s16(vreinterpretq_s16_u16(this->value));
  }
  xtext_really_inline int16_t min_val() const {
    return vminvq_s16(vreinterpretq_s16_u16(this->value));
  }
  // Order-sensitive comparisons
  xtext_really_inline simd16<int16_t>
...
template <> struct simd16<uint16_t> : base16_numeric<uint16_t> {
  ...
  // Array constructor
  xtext_really_inline simd16(const uint16_t *values) : simd16(load(values)) {}
  xtext_really_inline simd16(const char16_t *values)
      : simd16(load(reinterpret_cast<const uint16_t *>(values))) {}

  xtext_really_inline int16_t max_val() const { return vmaxvq_u16(*this); }
  xtext_really_inline int16_t min_val() const { return vminvq_u16(*this); }
  // Saturated math
  ...
  xtext_really_inline simd16<bool> bits_not_set() const {
  ...
  xtext_really_inline uint32_t sum() const { return vaddlvq_u16(value); }
};
...
template <>
xtext_really_inline uint64_t simd16x32<uint16_t>::not_in_range(
...
xtext_really_inline simd16<uint16_t> min(const simd16<uint16_t> a,
                                           simd16<uint16_t> b) {
