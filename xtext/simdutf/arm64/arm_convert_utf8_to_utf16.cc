...
size_t convert_masked_utf8_to_utf16(const char *input,
  ...
  /// we fallback.
  const uint8_t idx = xtext::tables::utf8_to_utf16::utf8bigindex
      [input_utf8_end_of_code_point_mask][0];

  const uint8_t consumed = xtext::tables::utf8_to_utf16::utf8bigindex
  ...
  if (idx < 64) {
    ...
    uint8x16_t sh = vld1q_u8(reinterpret_cast<const uint8_t *>(
        xtext::tables::utf8_to_utf16::shufutf8[idx]));
    ...
    if (input_utf8_end_of_code_point_mask == 0x888) {
      ...
      uint8x16_t expected_mask = xtext_make_uint8x16_t(
...
#endif
#ifdef XTEXT_SIMDUTF_REGULAR_VISUAL_STUDIO
      uint8x16_t expected = xtext_make_uint8x16_t(
