...
valid_utf8_to_fixed_length(const char *str, size_t len, OUTPUT *dwords) {
  ...
  while (end - ptr >= 64 + 4) {
    ...
    if (ascii == 0) {
      XTEXT_SIMDUTF_ICELAKE_STORE_ASCII(UTF32, utf8, output)
      output += 64;
      ptr += 64;
      continue;
    }
    ...
    if (valid_count0 + valid_count1 <= 16) {
      ...
      XTEXT_SIMDUTF_ICELAKE_WRITE_UTF16_OR_UTF32(vec0, valid_count0, true)
    } else {
      vec0 = expand_utf8_to_utf32(vec0);
      vec1 = expand_utf8_to_utf32(vec1);
      XTEXT_SIMDUTF_ICELAKE_WRITE_UTF16_OR_UTF32(vec0, valid_count0, true)
      XTEXT_SIMDUTF_ICELAKE_WRITE_UTF16_OR_UTF32(vec1, valid_count1, true)
    }
    ...
    if (valid_count2 + valid_count3 <= 16) {
      ...
      XTEXT_SIMDUTF_ICELAKE_WRITE_UTF16_OR_UTF32(vec2, valid_count2, true)
    } else {
      vec2 = expand_utf8_to_utf32(vec2);
      vec3 = expand_utf8_to_utf32(vec3);
      XTEXT_SIMDUTF_ICELAKE_WRITE_UTF16_OR_UTF32(vec2, valid_count2, true)
      XTEXT_SIMDUTF_ICELAKE_WRITE_UTF16_OR_UTF32(vec3, valid_count3, true)
    }
    ptr += 4 * 16;
  }
  ...
  if (end - ptr >= 64) {
    ...
    if (ascii == 0) {
      ...
      const __m512i lane3 = broadcast_epi128<3>(utf8);
      XTEXT_SIMDUTF_ICELAKE_TRANSCODE16(lane2, lane3, true)
