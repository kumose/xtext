...
fast_avx512_convert_utf8_to_utf16(const char *in, size_t len, char16_t *out) {
  ...
  while (result) {
    if (final_in - in >= 64) {
      result = process_block_utf8_to_utf16<XTEXT_SIMDUTF_FULL, big_endian>(
          in, out, final_in - in);
    } else if (in < final_in) {
      result = process_block_utf8_to_utf16<XTEXT_SIMDUTF_TAIL, big_endian>(
          in, out, final_in - in);
    } else {
      break;
    }
...
template <endianness big_endian>
xtext::result fast_avx512_convert_utf8_to_utf16_with_errors(const char *in,
                                                              size_t len,
                                                              char16_t *out) {
  ...
  if (!result) {
    ...
    return xtext::result(error_code::SUCCESS, out - init_out);
  }
}
