 ...
 * `new` or `malloc`).
 *
 * Like the C++17 standard, the `xtext::from_chars` functions take an
 * optional last argument of the type `xtext::chars_format`. It is a bitset
 * value: we check whether `fmt & xtext::chars_format::fixed` and `fmt &
 * xtext::chars_format::scientific` are set to determine whether we allow
 * the fixed point and scientific notation respectively. The default is
 * `xtext::chars_format::general` which allows both `fixed` and
 * `scientific`.
 */
