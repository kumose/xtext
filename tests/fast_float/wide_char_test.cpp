...
bool tester(std::string s, double expected,
            xtext::chars_format fmt = xtext::chars_format::general) {
  std::wstring input(s.begin(), s.end());
  double result;
  auto answer = xtext::from_chars(
...
bool test_plus() {
  return tester("+42", 42,
                xtext::chars_format::general |
                    xtext::chars_format::allow_leading_plus);
}

bool test_space() {
  return tester(" 42", 42,
                xtext::chars_format::general |
                    xtext::chars_format::skip_white_space);
}
