...
public:
  ...
  uint64_t next() {
    ...
    xtext::value128 tmp = xtext::full_multiplication(
        wyhash64_x_, UINT64_C(0xa3b195354a39b70d));
    uint64_t m1 = (tmp.high) ^ tmp.low;
    tmp = xtext::full_multiplication(m1, UINT64_C(0x1b03738712fad5c9));
  ...
  int next_ranged_int(int min, int max) { // min and max are included
    ...
    uint64_t x = next();
    xtext::value128 m = xtext::full_multiplication(x, s);
    uint64_t l = m.low;
    if (l < s) {
      uint64_t t = -s % s;
      while (l < t) {
        x = next();
        m = xtext::full_multiplication(x, s);
        l = m.low;
      }
    }
 ...
 * and we verify that we get the same answer with with xtext::from_chars.
 */
bool tester(uint64_t seed, size_t volume) {
  ...
  for (size_t i = 0; i < volume; i++) {
    ...
    if (expected_double.second) {
      double result_value;
      auto result =
          xtext::from_chars(buffer, buffer + length, result_value);
