...
void default_overload() {
  ...
  const int Q = 23;
  const double result = xtext::integer_times_pow10(W, Q);
...
void double_specialization() {
  const uint64_t W = 12345678901234567;
  const int Q = 23;
  const double result = xtext::integer_times_pow10<double>(W, Q);
  std::cout.precision(17);
...
void float_specialization() {
  ...
  const int Q = 23;
  const float result = xtext::integer_times_pow10<float>(W, Q);
