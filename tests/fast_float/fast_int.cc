...
/*
all tests conducted are to check xtext::from_chars functionality with int
...
int main() {
  ...
  for (std::size_t i = 0; i < int_basic_test.size(); ++i) {
    ...
    int result;
    auto answer = xtext::from_chars(f.data(), f.data() + f.size(), result);
  ...
  int result2;
  auto answer2 =
      xtext::from_chars(f2.data(), f2.data() + f2.size(), result2);
  ...
  int result3;
  auto answer3 =
      xtext::from_chars(f3.data(), f3.data() + f3.size(), result3);
  ...
  int result4;
  auto answer4 =
      xtext::from_chars(f4.data(), f4.data() + f4.size(), result4);
  ...
  int result5;
  auto answer5 =
      xtext::from_chars(f5.data(), f5.data() + f5.size(), result5);
  ...
  unsigned result6;
  auto answer6 =
      xtext::from_chars(f6.data(), f6.data() + f6.size(), result6);
  ...
  unsigned result7;
  auto answer7 =
      xtext::from_chars(f7.data(), f7.data() + f7.size(), result7);
  ...
  for (std::size_t i = 0; i < int_base_2_test.size(); ++i) {
    ...
    int result;
    auto answer =
        xtext::from_chars(f.data(), f.data() + f.size(), result, 2);
    if (answer.ec != std::errc()) {
      std::cerr << "could not convert to int for input: \"" << f << "\""
                << std::endl;
      return EXIT_FAILURE;
  ...
  for (std::size_t i = 0; i < base_octal_test.size(); ++i) {
    ...
    int result;
    auto answer =
        xtext::from_chars(f.data(), f.data() + f.size(), result, 8);
    if (answer.ec != std::errc()) {
      std::cerr << "could not convert to int for input: \"" << f << "\""
                << std::endl;
      return EXIT_FAILURE;
  ...
  for (std::size_t i = 0; i < base_hex_test.size(); ++i) {
    ...
    int result;
    auto answer =
        xtext::from_chars(f.data(), f.data() + f.size(), result, 16);
    if (answer.ec != std::errc()) {
      std::cerr << "could not convert to int for input: \"" << f << "\""
                << std::endl;
      return EXIT_FAILURE;
  ...
  for (std::size_t i = 0; i < invalid_base_test_1.size(); ++i) {
    ...
    int result;
    auto answer =
        xtext::from_chars(f.data(), f.data() + f.size(), result, -1);
    if (answer.ec != std::errc::invalid_argument) {
      std::cerr << "expected error should be 'invalid_argument' for: \"" << f
                << "\"" << std::endl;
      return EXIT_FAILURE;
  ...
  for (std::size_t i = 0; i < invalid_base_test_2.size(); ++i) {
    ...
    int result;
    auto answer =
        xtext::from_chars(f.data(), f.data() + f.size(), result, 37);
    if (answer.ec != std::errc::invalid_argument) {
      std::cerr << "expected error should be 'invalid_argument' for: \"" << f
                << "\"" << std::endl;
      return EXIT_FAILURE;
  ...
  // issue 235
  {
    ...
    s.shrink_to_fit();
    int foo;
    auto answer = xtext::from_chars(s.data(), s.data() + s.size(), foo);
    if (answer.ec != std::errc()) {
  ...
  // dont parse UTF-16 code units of emojis as int if low byte is ascii digit
  {
    ...
    for (size_t i = 0; i < array_size; i++) {
      auto e = emojis[i];
      int foo;
      auto answer = xtext::from_chars(e.data(), e.data() + e.size(), foo);
      if (answer.ec == std::errc()) {
        failed = true;
  ...
  // the uint8_t path and the 4-digit uint16_t SWAR path.
  {
    ...
    for (auto const &s : bad16) {
      uint8_t r8 = 123;
      auto a8 = xtext::from_chars(s.data(), s.data() + s.size(), r8);
      if (a8.ec == std::errc()) {
        failed = true;
      ...
      uint16_t r16 = 123;
      auto a16 = xtext::from_chars(s.data(), s.data() + s.size(), r16);
      if (a16.ec == std::errc()) {
        failed = true;
  ...
  // covers max_digits-1 (always in range) and max_digits+1 (always overflow).
  {
    ...
    auto verify = [&](std::string const &s, int base) -> bool {
      ...
      auto answer =
          xtext::from_chars(s.data(), s.data() + s.size(), result, base);
      ++checked;
      if (ov) {
        if (answer.ec != std::errc::result_out_of_range) {
          std::cerr << "base " << base
