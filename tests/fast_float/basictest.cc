...
TEST_CASE("double.rounds_to_nearest") {
  ...
  CHECK(xtext::detail::rounds_to_nearest() == false);
  ...
  CHECK(xtext::detail::rounds_to_nearest() == true);
#endif
}
...
TEST_CASE("double.parse_zero") {
  ...
  fesetround(FE_UPWARD);
  auto r1 = xtext::from_chars(zero, zero + 1, f);
  ...
  fesetround(FE_TOWARDZERO);
  auto r2 = xtext::from_chars(zero, zero + 1, f);
  ...
  fesetround(FE_DOWNWARD);
  auto r3 = xtext::from_chars(zero, zero + 1, f);
  ...
  fesetround(FE_TONEAREST);
  auto r4 = xtext::from_chars(zero, zero + 1, f);
...
TEST_CASE("double.parse_negative_zero") {
  ...
  fesetround(FE_UPWARD);
  auto r1 = xtext::from_chars(negative_zero, negative_zero + 2, f);
  CHECK(r1.ec == std::errc());
  ...
  fesetround(FE_TOWARDZERO);
  auto r2 = xtext::from_chars(negative_zero, negative_zero + 2, f);
  CHECK(r2.ec == std::errc());
  ...
  fesetround(FE_DOWNWARD);
  auto r3 = xtext::from_chars(negative_zero, negative_zero + 2, f);
  CHECK(r3.ec == std::errc());
  ...
  fesetround(FE_TONEAREST);
  auto r4 = xtext::from_chars(negative_zero, negative_zero + 2, f);
  CHECK(r4.ec == std::errc());
    ...
    if (newfile.is_open()) {
      ...
      while (std::getline(newfile, str)) {
        if (str.size() > 0) {
          ...
          auto xtext_r16 =
              xtext::from_chars(number_string, end_of_string, parsed_16);
          if (xtext_r16.ec != std::errc() &&
              xtext_r16.ec != std::errc::result_out_of_range) {
          ...
          float parsed_32{};
          auto xtext_r32 =
              xtext::from_chars(number_string, end_of_string, parsed_32);
          if (xtext_r32.ec != std::errc() &&
              xtext_r32.ec != std::errc::result_out_of_range) {
          ...
          double parsed_64{};
          auto xtext_r64 =
              xtext::from_chars(number_string, end_of_string, parsed_64);
          if (xtext_r64.ec != std::errc() &&
              xtext_r32.ec != std::errc::result_out_of_range) {
...
TEST_CASE("leading_zeroes") {
  ...
  CHECK(xtext::leading_zeroes(bit << 0) == 63);
  CHECK(xtext::leading_zeroes(bit << 1) == 62);
  CHECK(xtext::leading_zeroes(bit << 2) == 61);
  CHECK(xtext::leading_zeroes(bit << 61) == 2);
  CHECK(xtext::leading_zeroes(bit << 62) == 1);
  CHECK(xtext::leading_zeroes(bit << 63) == 0);
}

void test_full_multiplication(uint64_t lhs, uint64_t rhs, uint64_t expected_lo,
                              uint64_t expected_hi) {
  xtext::value128 v;
  v = xtext::full_multiplication(lhs, rhs);
  ...
  v = xtext::full_multiplication(rhs, lhs);
  CHECK_EQ(v.low, expected_lo);
  CHECK_EQ(v.high, expected_hi);
}
...
TEST_CASE("issue8") {
  ...
  for (int i = 0; i < 16; i++) {
    ...
    double d = 0.0;
    auto answer = xtext::from_chars(s, s + strlen(s) - i, d);
...
TEST_CASE("check_behavior") {
  ...
  double result;
  auto answer =
      xtext::from_chars(input.data(), input.data() + input.size(), result);
...
TEST_CASE("decimal_point_parsing") {
  ...
  xtext::parse_options options{};
  {
    ...
    answer = xtext::from_chars_advanced(
        input.data(), input.data() + input.size(), result, options);
    CHECK_MESSAGE(answer.ec == std::errc(), "expected parse success");
...
TEST_CASE("issue19") {
  ...
  answer = xtext::from_chars(answer.ptr + 1, input.data() + input.size(),
                                  result);
...
TEST_CASE("scientific_only") {
  ...
  auto answer =
      xtext::from_chars(input.data(), input.data() + input.size(), result,
                             xtext::chars_format::scientific);
  CHECK_MESSAGE(answer.ec != std::errc(),
                "It is not scientific! Parsed: " << result);

  input = "3.14e10";
  answer = xtext::from_chars(input.data(), input.data() + input.size(),
                                  result, xtext::chars_format::scientific);
  CHECK_MESSAGE(answer.ec == std::errc(),
...
TEST_CASE("powers_of_ten") {
  ...
  for (int i = start_point; i <= 308; ++i) {
    ...
    double actual;
    auto result = xtext::from_chars(buf, buf + 1000, actual);
    double expected =
...
#if FASTFLOAT_HAS_BIT_CAST
    if (xtext::cpp20_and_in_constexpr()) {
      using equiv_int = std::make_signed_t<xtext::equiv_uint_t<double>>;
...
constexpr void basic_test(std::string_view str, T expected,
  ...
  T actual{};
  auto result =
      xtext::from_chars(str.data(), str.data() + str.size(), actual);
  ...
  auto result16 = xtext::from_chars(u16, u16 + str.size(), actual);
  ...
  auto result32 = xtext::from_chars(u32, u32 + str.size(), actual);
...
constexpr void basic_test(std::string_view str, T expected,
                          xtext::parse_options options) {
  T actual{};
  auto result = xtext::from_chars_advanced(
...
T calculate_integer_times_pow10_expected_result(Int mantissa,
  ...
  T expected_result;
  const auto result = xtext::from_chars(
...
void verify_integer_times_pow10_dflt(Int mantissa, int decimal_exponent,
  ...
  const double actual =
      xtext::integer_times_pow10(mantissa, decimal_exponent);
...
void verify_integer_times_pow10(Int mantissa, int decimal_exponent,
  ...
  const auto actual =
      xtext::integer_times_pow10<T>(mantissa, decimal_exponent);
