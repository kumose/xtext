...
TEST(base64_fun) {
  ...
  for (size_t i = 0; i < sources.size(); i++) {
    ...
    std::vector<uint8_t> buffer(xtext::maximal_binary_length_from_base64(
        source.data(), source.size()));
    // convert to binary and check for errors
    xtext::result r = xtext::base64_to_binary(source.data(), source.size(),
                                                  (char *)buffer.data());
    if (r.error != xtext::error_code::SUCCESS) {
...
void check_xtext_result() { xtext::result r; }

// this is a compile test
int main_demo() {
  const char *source = "1234";
  // 4 == strlen(source)
  bool validutf8 = xtext::validate_utf8(source, 4);
  if (validutf8) {
    puts("valid UTF-8");
  } else {
    puts("invalid UTF-8");
  ...
  size_t expected_utf16words = xtext::utf16_length_from_utf8(source, 4);
  std::unique_ptr<char16_t[]> utf16_output{new char16_t[expected_utf16words]};
  // convert to UTF-16LE
  size_t utf16words =
      xtext::convert_utf8_to_utf16le(source, 4, utf16_output.get());
  printf("wrote %zu UTF-16LE code units.", utf16words);
  // It wrote utf16words * sizeof(char16_t) bytes.
  bool validutf16 = xtext::validate_utf16le(utf16_output.get(), utf16words);
  if (validutf16) {
    puts("valid UTF-16LE");
  } else {
  ...
  size_t expected_utf8words =
      xtext::utf8_length_from_utf16le(utf16_output.get(), utf16words);
  std::unique_ptr<char[]> utf8_output{new char[expected_utf8words]};
  // convert to UTF-8
  size_t utf8words = xtext::convert_utf16le_to_utf8(
...
TEST(utf8_streaming) {
  ...
  length = xtext::trim_partial_utf8(unicode, length);
  // Now we can transcode safely
  size_t budget_utf16 = xtext::utf16_length_from_utf8(unicode, length);
  std::unique_ptr<char16_t[]> utf16{new char16_t[budget_utf16]};
  size_t utf16words =
      xtext::convert_utf8_to_utf16le(unicode, length, utf16.get());
  // We can then transcode the next batch
  const char *next = unicode + length;
  size_t next_length = sizeof(unicode) - length;
  size_t next_budget_utf16 = xtext::utf16_length_from_utf8(next, next_length);
  std::unique_ptr<char16_t[]> next_utf16{new char16_t[next_budget_utf16]};
  size_t next_utf16words =
      xtext::convert_utf8_to_utf16le(next, next_length, next_utf16.get());
...
TEST(issue829) {
  ...
  size_t length = 1;
  length = xtext::trim_partial_utf16le(unicode, length);
...
TEST(utf16_streaming) {
  ...
  length = xtext::trim_partial_utf16le(unicode, length);
  // Now we can transcode safely
  size_t budget_utf8 = xtext::utf8_length_from_utf16le(unicode, length);
  std::unique_ptr<char[]> utf8{new char[budget_utf8]};
  size_t utf8words =
      xtext::convert_utf16le_to_utf8(unicode, length, utf8.get());
  // We can then transcode the next batch
  const char16_t *next = unicode + length;
  size_t next_length = 6 - length;
  size_t next_budget_utf8 =
      xtext::utf8_length_from_utf16le(next, next_length);
  std::unique_ptr<char[]> next_utf8{new char[next_budget_utf8]};
  size_t next_utf8words =
      xtext::convert_utf16le_to_utf8(next, next_length, next_utf8.get());
...
TEST(error_location_badascii) {
  ...
  if (res.error != xtext::error_code::SUCCESS) {
...
TEST(error_location_badutf8) {
  ...
  xtext::result res = implementation.validate_utf8_with_errors(
      bad_utf8.data(), bad_utf8.size());
  if (res.error != xtext::error_code::SUCCESS) {
    printf("error at index %zu\n", res.count);
  }
  ASSERT_EQUAL(res.error, xtext::error_code::HEADER_BITS);
  ASSERT_EQUAL(res.count, 5);
...
TEST(error_location_badutf8_transcoding) {
  ...
  std::string bad_utf8 = "\xc3\xa9\xc3\xa9\x20\xff\xc3\xa9";
  size_t budget_utf16 =
      xtext::utf16_length_from_utf8(bad_utf8.data(), bad_utf8.size());
  std::unique_ptr<char16_t[]> utf16{new char16_t[budget_utf16]};

  xtext::result res = xtext::convert_utf8_to_utf16_with_errors(
  ...
  ASSERT_EQUAL(res.error, xtext::error_code::HEADER_BITS);
  ASSERT_EQUAL(res.count, 5);
  res = xtext::convert_utf8_to_utf16_with_errors(bad_utf8.data(), res.count,
                                                   utf16.get());
  if (res.error == xtext::error_code::SUCCESS) {
...
TEST(compile_time_trim_partial_utf8) {
  ...
  constexpr auto compile_time = xtext::trim_partial_utf8(str);
  const auto runtime = xtext::trim_partial_utf8(str);
...
TEST(compile_time_trim_partial_utf16) {
  ...
  constexpr auto compile_time = xtext::trim_partial_utf16(str);
  const auto runtime = xtext::trim_partial_utf16(str);
  ASSERT_EQUAL(compile_time, runtime);
}
...
TEST(compile_time_trim_partial_utf16le) {
  ...
  constexpr auto compile_time = xtext::trim_partial_utf16le(str);
  const auto runtime = xtext::trim_partial_utf16le(str);
  ASSERT_EQUAL(compile_time, runtime);
}
...
TEST(compile_time_trim_partial_utf16be) {
  ...
  constexpr auto compile_time = xtext::trim_partial_utf16be(str);
  const auto runtime = xtext::trim_partial_utf16be(str);
  ASSERT_EQUAL(compile_time, runtime);
}
#endif
