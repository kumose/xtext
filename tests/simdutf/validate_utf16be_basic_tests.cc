...
#include "tests/simdutf/helpers/compiletime_conversions.h"
...
TEST_LOOP(validate_utf16be_ascii) {
  xtext::tests::helpers::random_utf16 generator{seed, 1, 0};
...
TEST_LOOP(validate_utf16be_returns_true_for_valid_input_surrogate_pairs_short) {
  xtext::tests::helpers::random_utf16 generator{seed, 0, 1};
...
TEST(validate_utf16be_returns_true_for_valid_input_mixed) {
  uint32_t seed{1234};
  xtext::tests::helpers::random_utf16 generator{seed, 1, 1};
  const auto utf16{generator.generate_be(512)};
  ASSERT_TRUE(implementation.validate_utf16be(
      reinterpret_cast<const char16_t *>(utf16.data()), utf16.size()));
}
...
TEST(compile_time_validation) {
  ...
  static_assert(xtext::validate_utf16be(u"hello!"_utf16be));
  ...
  static_assert(not xtext::validate_utf16be(invalid_be));
}

TEST(compile_time_ascii_validation_native_endian) {
  using namespace xtext::tests::helpers;
  static_assert(
      xtext::validate_utf16_as_ascii(u"I am ascii, I promise!"_utf16));
  static_assert(
      not xtext::validate_utf16_as_ascii(u"But this isn't: köttbulle"_utf16));
}

TEST(compile_time_ascii_validation_be) {
  using namespace xtext::tests::helpers;
  static_assert(
      xtext::validate_utf16be_as_ascii(u"I am ascii, I promise!"_utf16be));
  static_assert(not xtext::validate_utf16be_as_ascii(
