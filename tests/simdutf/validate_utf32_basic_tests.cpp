...
#include "tests/simdutf/helpers/random_utf32.h"
#include "tests/simdutf/helpers/test.h"

TEST_LOOP(validate_utf32_returns_true_for_valid_input) {
  xtext::tests::helpers::random_utf32 generator{seed};
...
TEST(compile_time_validate) {
  ...
  static_assert(xtext::validate_utf32(good));

  constexpr auto bad = make_bad();
  static_assert(not xtext::validate_utf32(bad));
}
