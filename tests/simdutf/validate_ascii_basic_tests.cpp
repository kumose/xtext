...
concept passable_to_validate =
    requires(InputPtr p) { xtext::scalar::ascii::validate(p, 10u); };

} // namespace

TEST(compile_time_valid) {
  static_assert(std::ranges::all_of(goodsequences, [](std::string_view input) {
    return xtext::validate_ascii(input);
  }));

  using namespace xtext::tests::helpers;
  constexpr auto ascii = "a normal ascii text"_latin1;

  static_assert(xtext::validate_ascii(ascii));
  static_assert(xtext::validate_ascii(ascii.as_array<unsigned char>()));
  static_assert(xtext::validate_ascii(ascii.as_array<signed char>()));
  static_assert(xtext::validate_ascii(ascii.as_array<std::byte>()));
