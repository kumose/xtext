...
#ifndef XTEXT_SIMDUTF_BASE64_TEST_MAXLEN
  #error "XTEXT_SIMDUTF_BASE64_TEST_MAXLEN not set."
#endif
constexpr std::size_t max_len = XTEXT_SIMDUTF_BASE64_TEST_MAXLEN;
...
size_t prefix_length_base64_index(size_t count, xtext::base64_options options,
                                  const char *input, size_t length) {
  size_t i = 0;
  while (i < length && xtext::base64_ignorable(input[i], options)) {
    i++;
  }
  if (count == 0) {
    return i; // duh!
  ...
  for (; i < length; i++) {
    if (xtext::base64_ignorable(input[i], options)) {
      continue;
    }
...
size_t length_without_empty_tail(
    ...
    xtext::base64_options options = xtext::base64_options::base64_default) {
  if (input.size() == 0) {
    return 0;
  }
  size_t i = input.size();
  while (i > 0 && xtext::base64_ignorable(input[i - 1], options)) {
    i--;
  }
  return i;
}
...
#define XTEXT_SIMDUTF_BASE64URL_TESTS 1
...
TEST(tc39_illegal_padded_chunks) {
  ...
  for (const std::string &input : test_cases) {
    ...
    for (auto option :
         {xtext::last_chunk_handling_options::strict,
          xtext::last_chunk_handling_options::loose,
          xtext::last_chunk_handling_options::stop_before_partial}) {
      auto r = xtext::base64_to_binary_safe(
          input.data(), input.size(), reinterpret_cast<char *>(back.data()),
          len, xtext::base64_default, option, true);
      ASSERT_FALSE(r.error == xtext::error_code::SUCCESS);
      ASSERT_FALSE(r.error == xtext::error_code::OUTPUT_BUFFER_TOO_SMALL);
    }
  }
}
...
TEST(issue_node_anything_goes) {
  ...
  auto test =
      [](const std::string &input, const std::string &expected,
         xtext::base64_options options =
             xtext::base64_options::base64_default_or_url_accept_garbage) {
        size_t buflen = xtext::maximal_binary_length_from_base64(
            input.data(), input.size());
        std::vector<char> back(buflen);
        size_t written_len = buflen;
        auto result = xtext::base64_to_binary_safe(
            input.data(), input.length(), back.data(), written_len,
            xtext::base64_default_or_url_accept_garbage);
        ASSERT_EQUAL(result.error, xtext::error_code::SUCCESS);
...
TEST(issue_dash) {
  ...
  size_t len = back.size();
  auto r = xtext::base64_to_binary_safe(input.data(), input.size(),
                                          back.data(), len);
  ASSERT_EQUAL(r.error, xtext::error_code::SUCCESS);
...
TEST(issue_dash_partial) {
  ...
  size_t len = back.size();
  auto r = xtext::base64_to_binary_safe(
      input.data(), input.size(), back.data(), len, xtext::base64_default,
      xtext::last_chunk_handling_options::stop_before_partial);
  ASSERT_EQUAL(r.error, xtext::error_code::SUCCESS);
  ASSERT_EQUAL(r.count, 4);
  ASSERT_EQUAL(len, 1);
...
TEST(tc39_4a) {
  ...
  for (const auto tc : test_cases) {
    ...
    auto r = xtext::base64_to_binary_safe(
        ...
        xtext::base64_default,
        xtext::last_chunk_handling_options::stop_before_partial, true);
    ASSERT_EQUAL(r.error, xtext::error_code::SUCCESS);
...
TEST(tc39_3a) {
  ...
  auto r = xtext::base64_to_binary_safe(
      ...
      xtext::base64_default,
      xtext::last_chunk_handling_options::stop_before_partial, true);
  ASSERT_TRUE(r.error != xtext::error_code::SUCCESS);
}
  ...
  for (const std::string &input : test_cases) {
    ...
    for (auto option :
         {xtext::last_chunk_handling_options::strict,
          xtext::last_chunk_handling_options::loose,
          xtext::last_chunk_handling_options::stop_before_partial}) {
      auto r = xtext::base64_to_binary(input.data(), input.size(),
                                         reinterpret_cast<char *>(back.data()),
                                         xtext::base64_default, option);
      ASSERT_FALSE(r.error == xtext::error_code::SUCCESS);
    }
  }
...
TEST(with_lines_pauldreik) {
  ...
  for (auto selected_option : {xtext::base64_options::base64_url,
                               xtext::base64_options::base64_default}) {
    for (size_t line_length = 5; line_length < 128; line_length += 7) {
      for (size_t length = 1; length < 2048; length += 17) {
        buffer.resize(xtext::base64_length_from_binary_with_lines(
...
TEST(with_lines) {
  ...
  for (size_t line_length : {4, 64, 76, 80, 128}) {
    for (size_t len = 0; len < max_len; len += 17) {
      ...
      std::vector<char> buffer;
      buffer.resize(xtext::base64_length_from_binary_with_lines(
          len, xtext::base64_default, line_length));
      std::mt19937 gen((std::mt19937::result_type)(seed));
      ...
      for (size_t trial = 0; trial < 2; trial++) {
        ...
        ASSERT_TRUE(verify_lines(buffer.data(), buffer.size(), line_length));
        std::vector<char> back(xtext::maximal_binary_length_from_base64(
            buffer.data(), buffer.size()));
        auto option = xtext::last_chunk_handling_options::stop_before_partial;

        xtext::full_result r = implementation.base64_to_binary_details(
            buffer.data(), buffer.size(), back.data(), xtext::base64_default,
            option);
        ASSERT_EQUAL(r.error, xtext::error_code::SUCCESS);
        const size_t expected_output_length = len;
        const size_t expected_input_length = prefix_length_base64_index(
            (expected_output_length + 2) / 3 * 4, xtext::base64_default,
            buffer.data(), buffer.size());
...
TEST(partial_should_decode_up_to_last_chunk) {
  for (size_t len = 0; len < max_len; len++) {
    std::vector<char> source(len, 0);
    std::vector<char> buffer;
    buffer.resize(xtext::base64_length_from_binary(len));
    std::mt19937 gen((std::mt19937::result_type)(seed));
    std::uniform_int_distribution<int> byte_generator{0, 255};
    for (size_t trial = 0; trial < 10; trial++) {
      ...
      if (buffer.size() > 0 && len > 0 && len % 3 != 0) {
        ...
        for (size_t i = buffer.size() - 1;; i--) {
          if (xtext::base64_valid(buffer[i], xtext::base64_default)) {
            buffer[i] = ' ';
            to_remove--;
            if (to_remove == 0) {
              break;
        ...
        r = implementation.base64_to_binary_details(
            buffer.data(), buffer.size(), back.data(), xtext::base64_default,
            option);
        ASSERT_EQUAL(r.error, xtext::error_code::BASE64_INPUT_REMAINDER);
        ...
        r = implementation.base64_to_binary_details(
            buffer.data(), buffer.size(), back.data(), xtext::base64_default,
            option);
        ASSERT_EQUAL(r.error, xtext::error_code::INVALID_BASE64_CHARACTER);
      }
    }
  }
}
...
TEST(partial_should_decode_four_wise_chunks) {
  ...
  auto r = implementation.base64_to_binary_details(
      input.data(), input.size(), buffer.data(), xtext::base64_default,
      option);
  ASSERT_EQUAL(r.error, xtext::error_code::SUCCESS);
...
TEST(stop_before_partial_one_char) {
  ...
  // https://tc39.es/proposal-arraybuffer-base64/spec/#sec-frombase64
  xtext::result r = implementation.base64_to_binary(
      base64.data(), base64.size(), back.data(), xtext::base64_default,
      xtext::last_chunk_handling_options::stop_before_partial);
  ASSERT_EQUAL(r.error, xtext::error_code::SUCCESS);
  ASSERT_EQUAL(r.count, 0);
  size_t buflen = back.size();
  ASSERT_EQUAL(buflen, 0);
  r = xtext::base64_to_binary_safe(
...
TEST(hybrid_decoding) {
  ...
  for (const auto &test : test_data) {
    ...
    std::vector<uint8_t> decoded(xtext::maximal_binary_length_from_base64(
        base64.data(), base64.size()));
    auto r = implementation.base64_to_binary(
        base64.data(), base64.size(), reinterpret_cast<char *>(decoded.data()),
        xtext::base64_default_or_url);
    ASSERT_EQUAL(r.error, xtext::error_code::SUCCESS);
...
TEST(roundtrip_base64_with_garbage) {
  for (size_t len = 0; len < max_len; len++) {
    ...
    for (size_t trial = 0; trial < 10; trial++) {
      ...
      std::vector<char> back(xtext::maximal_binary_length_from_base64(
          buffer.data(), buffer.size()));
      for (auto option : {xtext::last_chunk_handling_options::strict,
                          xtext::last_chunk_handling_options::loose}) {
        xtext::result r = implementation.base64_to_binary(
            buffer.data(), buffer.size(), back.data(),
            xtext::base64_default_accept_garbage, option);
        ASSERT_EQUAL(r.error, xtext::error_code::SUCCESS);
        ASSERT_EQUAL(r.count, len);
        ASSERT_TRUE(
      ...
      for (auto option :
        ...
        ASSERT_EQUAL(back_length, len);
        for (size_t i = r.count; i < buffer.size(); i++) {
          ASSERT_TRUE(!xtext::base64_valid(
              buffer[i], xtext::base64_default_accept_garbage));
        }
        ASSERT_TRUE(std::equal(back.begin(), back.begin() + back_length,
                               source.begin()));
      }
...
TEST(roundtrip_base64_url_with_garbage) {
  for (size_t len = 0; len < max_len; len++) {
    ...
    for (size_t trial = 0; trial < 10; trial++) {
      ...
      size_t size = implementation.binary_to_base64(
          source.data(), source.size(), buffer.data(), xtext::base64_url);
      buffer.resize(size);
      for (size_t i = 0; i < 5; i++) {
      ...
      for (auto option :
        ...
        xtext::result r = implementation.base64_to_binary(
            buffer.data(), buffer.size(), back.data(),
            xtext::base64_url_accept_garbage, option);
        ASSERT_EQUAL(r.error, xtext::error_code::SUCCESS);
        ASSERT_EQUAL(r.count, len);
        ASSERT_TRUE(
            std::equal(back.begin(), back.begin() + len, source.begin()));
...
TEST(roundtrip_base64_with_lots_of_spaces_at_the_end) {
  ...
  for (size_t len = 0; len < max_len; len += 10) {
    std::vector<char> source(len, 0);
    buffer.clear();
    buffer.resize(xtext::base64_length_from_binary(len), '\0');
    std::mt19937 gen((std::mt19937::result_type)(seed));
    std::uniform_int_distribution<int> byte_generator{0, 255};
    for (size_t trial = 0; trial < 10; trial++) {
      for (size_t i = 0; i < len; i++) {
...
TEST(base64_decode_just_one_padding_partial_safe) {
  std::vector<std::tuple<std::string, xtext::result, size_t>> test_cases = {
      {"uuuu             uu=", {xtext::error_code::SUCCESS, 4}, 3},
      // 5. If char is "=", then If chunkLength < 2, then Let error be a new
      // SyntaxError exception.
      {"uuuu             u==",
       {xtext::error_code::BASE64_INPUT_REMAINDER, 18},
       3}, // error
      {"uuuu             u=",
       {xtext::error_code::BASE64_INPUT_REMAINDER, 18},
       3}, // error
      {"uuuu             ==",
       {xtext::error_code::INVALID_BASE64_CHARACTER, 17},
       3}, // error
      {"uuuu             =",
       {xtext::error_code::INVALID_BASE64_CHARACTER, 17},
       3}, // error
  ...
  for (auto &p : test_cases) {
    ...
    printf("input: %s\n", input.c_str());
    for (auto option : {xtext::base64_options::base64_default,
                        xtext::base64_options::base64_url}) {
...
TEST(base64_decode_just_one_padding_partial_generous) {
  std::vector<std::pair<std::string, xtext::result>> test_cases = {
      {"uuuu             uu=", {xtext::error_code::SUCCESS, 3}},
      // 5. If char is "=", then If chunkLength < 2, then Let error be a new
      // SyntaxError exception.
      {"uuuu             u==",
       {xtext::error_code::BASE64_INPUT_REMAINDER, 18}}, // error
      {"uuuu             u=",
       {xtext::error_code::BASE64_INPUT_REMAINDER, 18}}, // error
      {"uuuu             ==",
       {xtext::error_code::INVALID_BASE64_CHARACTER, 17}}, // error
      {"uuuu             =",
       {xtext::error_code::INVALID_BASE64_CHARACTER, 17}}, // error
  };
...
TEST(base64_decode_just_one_padding_loose) {
  ...
  for (auto &p : test_cases) {
    ...
    for (auto option : {xtext::base64_options::base64_default,
                        xtext::base64_options::base64_url}) {
      for (auto chunk_option : {xtext::last_chunk_handling_options::loose}) {
        auto result = implementation.base64_to_binary(
            input.data(), input.size(), buffer.data(), option, chunk_option);
        ASSERT_EQUAL(result.error, expected_result.error);
        ASSERT_EQUAL(result.count, expected_result.count);
...
TEST(base64_decode_just_one_padding_strict) {
  ...
  for (auto &p : test_cases) {
    ...
    for (auto option : {xtext::base64_options::base64_default,
                        xtext::base64_options::base64_url}) {
      for (auto chunk_option : {xtext::last_chunk_handling_options::strict}) {
        auto result = implementation.base64_to_binary(
            input.data(), input.size(), buffer.data(), option, chunk_option);
        ASSERT_EQUAL(result.error, expected_result.error);
        ASSERT_EQUAL(result.count, expected_result.count);
...
TEST(base64_decode_partial_cases) {
  std::vector<std::pair<std::string, xtext::result>> test_cases = {
      {"ZXhhZg", {xtext::error_code::SUCCESS, 4}},
      {"ZXhhZg                                                               "
       " ",
       {xtext::error_code::SUCCESS, 4}},
      {"                                                                "
       "ZXhhZg",
       {xtext::error_code::SUCCESS, 68}},
  };
  std::vector<char> buffer(3);
  for (auto &p : test_cases) {
    auto input = p.first;
...
TEST(base64_decode_strict_cases) {
  std::vector<std::pair<std::string, uint64_t>> test_cases = {
      {"ZXhhZg==", xtext::error_code::SUCCESS},
      {"YWE=", xtext::error_code::SUCCESS},
      {"YWF=", xtext::error_code::BASE64_EXTRA_BITS},
      {"ZXhhZh==", xtext::error_code::BASE64_EXTRA_BITS},
      // {"ZXhhZg", xtext::error_code::BASE64_INPUT_REMAINDER},
      // {"ZXhhZh", xtext::error_code::BASE64_INPUT_REMAINDER},
      {"Z   X  h  h   Z h =   =", xtext::error_code::BASE64_EXTRA_BITS},
      //{"ZX  h  hZg", xtext::error_code::BASE64_INPUT_REMAINDER},
      //{"ZXh  hZ  h", xtext::error_code::BASE64_INPUT_REMAINDER},
  };
  std::vector<char> buffer(1024);
  for (auto &p : test_cases) {
    auto input = p.first;
    auto expected_error = p.second;
    xtext::result result = implementation.base64_to_binary(
        input.data(), input.size(), buffer.data(), xtext::base64_default,
        xtext::last_chunk_handling_options::strict);
    ASSERT_EQUAL(result.error, expected_error);
    size_t written = buffer.size();
    result = xtext::base64_to_binary_safe(
        input.data(), input.size(), buffer.data(), written,
        xtext::base64_default, xtext::last_chunk_handling_options::strict);
    ASSERT_EQUAL(result.error, expected_error);
  }
}
...
TEST(base64_decode_strict_cases_length) {
  std::vector<std::pair<std::string, xtext::result>> test_cases = {
      {"ddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd"
       ...
       {xtext::error_code::BASE64_EXTRA_BITS, 131}},
  };
  std::vector<char> buffer(1024);
  for (auto &p : test_cases) {
    auto input = p.first;
...
TEST(issue_webkit_290829) {
  ...
  for (auto option :
    ...
    const auto r1 =
        implementation.base64_to_binary(data.data(), data.size(), output.data(),
                                        xtext::base64_default, option);
    ASSERT_EQUAL(r1.error, xtext::error_code::INVALID_BASE64_CHARACTER);
  ...
  for (auto option :
    ...
    auto r = xtext::base64_to_binary_safe(
        ...
        xtext::base64_default, option, decode_up_to_bad_char);
...
TEST(issue_615) {
  ...
  const auto r1 =
      implementation.base64_to_binary(data.data(), data.size(), output.data(),
                                      xtext::base64_default, xtext::strict);
  ASSERT_EQUAL(r1.error, xtext::error_code::INVALID_BASE64_CHARACTER);
  ASSERT_EQUAL(r1.count, 1);
  const auto r2 = implementation.base64_to_binary(
      data.data() + 1, data.size() - 1, output.data(), xtext::base64_default,
      xtext::strict);
  ASSERT_EQUAL(r2.error, xtext::error_code::INVALID_BASE64_CHARACTER);
  ASSERT_EQUAL(r2.count, 0);
  const auto r3 = implementation.base64_to_binary(
      data.data(), data.size(), output.data(), xtext::base64_default,
      xtext::stop_before_partial);
  ASSERT_EQUAL(r3.error, xtext::error_code::INVALID_BASE64_CHARACTER);
  ASSERT_EQUAL(r3.count, 1);
  const auto r4 = implementation.base64_to_binary(
      data.data() + 1, data.size() - 1, output.data(), xtext::base64_default,
      xtext::stop_before_partial);
  ASSERT_EQUAL(r4.error, xtext::error_code::INVALID_BASE64_CHARACTER);
...
TEST(base64_decode_complete_input) {
  ...
  for (auto option :
    ...
    auto result = implementation.base64_to_binary(
        ...
        output_buffer.data(), xtext::base64_default, option);
...
TEST(issue_824_a) {
  ...
  const auto result = xtext::base64_to_binary_safe(
      (const char *)base64.data(), base64.size(), outbuf.data(), outlen,
      xtext::base64_default,
      xtext::last_chunk_handling_options::stop_before_partial, false);
  ASSERT_EQUAL(result.error, xtext::error_code::SUCCESS);
  ...
  for (auto option :
    ...
    for (const std::string &input_data : test_cases) {
      ...
      auto result = implementation.base64_to_binary(
          ...
          reinterpret_cast<char *>(output_buffer.data()),
          xtext::base64_default, option);
      if (option == xtext::last_chunk_handling_options::strict) {
        ASSERT_EQUAL(result.error, xtext::error_code::BASE64_INPUT_REMAINDER);
      } else {
        ASSERT_EQUAL(result.error, xtext::error_code::SUCCESS);
        ASSERT_EQUAL(result.count, 0);
      }
...
TEST(base64_decode_webkit_like_but_random_more_cases) {
  for (size_t len = 1; len <= max_len; len++) {
    for (size_t trial = 0; trial < 10; trial++) {
      ...
      xtext_maybe_unused const size_t size = implementation.binary_to_base64(
          source.data(), source.size(), buffer.data());
        ...
        for (auto option :
          ...
          if (option == xtext::last_chunk_handling_options::strict) {
            ASSERT_EQUAL(result.error,
                         xtext::error_code::BASE64_INPUT_REMAINDER);
          } else {
            ASSERT_EQUAL(result.error, xtext::error_code::SUCCESS);
      ...
      for (size_t removed = 1; !buffer.empty() && removed <= 2; removed++) {
        ...
        for (auto option :
             {xtext::last_chunk_handling_options::strict,
              xtext::last_chunk_handling_options::stop_before_partial}) {
          std::vector<uint8_t> output_buffer(
              xtext::maximal_binary_length_from_base64(buffer.data(),
                                                         buffer.size()));
          size_t written = output_buffer.size();
          auto result = xtext::base64_to_binary_safe(
              buffer.data(), buffer.size(),
...
TEST(base64_decode_strict_mode) {
  ...
  for (const std::pair<std::string, std::string> &t : test_cases) {
    ...
    auto result = implementation.base64_to_binary(
        input_data.data(), input_data.size(),
        reinterpret_cast<char *>(output_buffer.data()), xtext::base64_default,
        xtext::last_chunk_handling_options::strict);
...
TEST(issue_520_url) {
  ...
  const auto r = implementation.base64_to_binary(
      (const char *)data.data(), data.size(), out.data(), xtext::base64_url);
  ASSERT_EQUAL(r.error, xtext::error_code::BASE64_INPUT_REMAINDER);
  ASSERT_EQUAL(r.count, 64);
}
...
TEST(issue_509) {
  ...
  const auto r = implementation.base64_to_binary(
      data.data(), data.size(), out.data(), xtext::base64_default);
  ASSERT_EQUAL(r.error, xtext::error_code::INVALID_BASE64_CHARACTER);
...
TEST(decode_non_ascii_utf16) {
  ...
  std::vector<xtext::error_code> codes = {
      xtext::error_code::INVALID_BASE64_CHARACTER};
  ...
  for (size_t i = 0; i < cases.size(); i++) {
    ...
    r = xtext::base64_to_binary_safe(cases[i].data(), cases[i].size(),
                                       buffer.data(), len);
    ASSERT_EQUAL(r.error, codes[i]);
    ASSERT_EQUAL(r.count, counts[i]);
  }
...
TEST(decode_base64_cases) {
  ...
  std::vector<xtext::error_code> codes = {xtext::error_code::SUCCESS};
...
TEST(complete_safe_decode_base64_cases) {
  for (const auto &p : cases::whitespaces) {
    ...
    bufsize = buffer.size();
    r = xtext::atomic_base64_to_binary_safe(p.second.data(), p.second.size(),
                                              buffer.data(), bufsize);
    ASSERT_EQUAL(r.error, xtext::error_code::SUCCESS);
    ASSERT_EQUAL(bufsize, p.first.size());
    for (size_t i = 0; i < bufsize; i++) {
...
#if XTEXT_SIMDUTF_ATOMIC_REF
    bufsize = buffer.size();
    r = xtext::base64_to_binary_safe(p.second.data(), p.second.size(),
                                       buffer.data(), bufsize,
                                       xtext::base64_url);
    ASSERT_EQUAL(r.error, xtext::error_code::SUCCESS);
    ASSERT_EQUAL(bufsize, p.first.size());
...
TEST(encode_base64_cases) {
  for (const auto &p : cases::simple) {
    std::vector<char> buffer(
        xtext::base64_length_from_binary(p.first.size()));
...
TEST(encode_base64_cases_no_padding) {
  for (const auto &p : cases::no_padding) {
    std::vector<char> buffer(xtext::base64_length_from_binary(
        p.first.size(), xtext::base64_default_no_padding));
    ASSERT_EQUAL(buffer.size(), p.second.size());
    size_t s = implementation.binary_to_base64(
        p.first.data(), p.first.size(), buffer.data(),
        xtext::base64_default_no_padding);
    ASSERT_EQUAL(s, p.second.size());
    ASSERT_EQUAL(std::string(buffer.data(), buffer.size()), p.second);
  }
}

#if XTEXT_SIMDUTF_BASE64URL_TESTS

TEST(encode_base64url_simple) {
  for (const auto &p : cases::simple_url) {
    std::vector<char> buffer(xtext::base64_length_from_binary(
        p.first.size(), xtext::base64_url));
    ASSERT_EQUAL(buffer.size(), p.second.size());
    size_t s = implementation.binary_to_base64(
        p.first.data(), p.first.size(), buffer.data(), xtext::base64_url);
    ASSERT_EQUAL(s, p.second.size());
...
TEST(decode_base64url) {
  for (const auto &p : cases::simple_url) {
    ...
    ASSERT_EQUAL(buffer.size(), p.first.size());
    xtext::result r = implementation.base64_to_binary(
        p.second.data(), p.second.size(), buffer.data(), xtext::base64_url);
    ASSERT_EQUAL(r.error, xtext::error_code::SUCCESS);
    ASSERT_EQUAL(r.count, p.first.size());
    for (size_t i = 0; i < buffer.size(); i++) {
      ASSERT_EQUAL(buffer[i], p.first[i]);
...
TEST(encode_base64url_with_padding_cases) {
  for (const auto &p : cases::simple_url_with_padding) {
    std::vector<char> buffer(xtext::base64_length_from_binary(
        p.first.size(), xtext::base64_url_with_padding));
    ASSERT_EQUAL(buffer.size(), p.second.size());
    size_t s = implementation.binary_to_base64(
        p.first.data(), p.first.size(), buffer.data(),
        xtext::base64_url_with_padding);
...
TEST(roundtrip_base64) {
  for (size_t len = 0; len < max_len; len++) {
    ...
    for (size_t trial = 0; trial < 10; trial++) {
      ...
      size_t size = implementation.binary_to_base64(
          source.data(), source.size(), buffer.data());
      ASSERT_EQUAL(size, xtext::base64_length_from_binary(len));
      xtext::result r =
...
TEST(roundtrip_base64url) {
  for (size_t len = 0; len < max_len; len++) {
    std::vector<char> source(len, 0);
    std::vector<char> buffer;
    buffer.resize(xtext::base64_length_from_binary(len, xtext::base64_url));
    std::vector<char> back(len);
    std::mt19937 gen((std::mt19937::result_type)(seed));
    std::uniform_int_distribution<int> byte_generator{0, 255};
    for (size_t trial = 0; trial < 10; trial++) {
      ...
      size_t size = implementation.binary_to_base64(
          source.data(), source.size(), buffer.data(), xtext::base64_url);
      ASSERT_EQUAL(
          size, xtext::base64_length_from_binary(len, xtext::base64_url));
      xtext::result r = implementation.base64_to_binary(
          buffer.data(), size, back.data(), xtext::base64_url);
      ASSERT_EQUAL(r.error, xtext::error_code::SUCCESS);
      ASSERT_EQUAL(r.count, len);
      if (back != source) {
        printf("=====input size %zu\n", len);
      ...
      for (auto option :
        ...
        if ((size % 4) == 0) {
          ...
          if (option == xtext::last_chunk_handling_options::strict) {
            ASSERT_EQUAL(r.error, xtext::error_code::BASE64_INPUT_REMAINDER);
          } else if (option == xtext::last_chunk_handling_options::loose) {
            ASSERT_EQUAL(r.error, xtext::error_code::SUCCESS);
            ASSERT_EQUAL(r.count, len);
            ASSERT_TRUE(back == source);
          } else if (option == xtext::last_chunk_handling_options::
                                   stop_before_partial) {
            ASSERT_EQUAL(r.error, xtext::error_code::SUCCESS);
  ...
  for (size_t len = 0; len < max_len; len++) {
    ...
    for (size_t trial = 0; trial < 10; trial++) {
      ...
      ASSERT_EQUAL(
          size, xtext::base64_length_from_binary(len, xtext::base64_url));
      xtext::result r = implementation.base64_to_binary(
          buffer16.data(), size, back.data(), xtext::base64_url);
      ASSERT_EQUAL(r.error, xtext::error_code::SUCCESS);
      ASSERT_EQUAL(r.count, len);
      if (back != source) {
        printf("=====input size %zu\n", len);
...
TEST(bad_padding_base64) {
  for (size_t len = 0; len < max_len; len++) {
    std::vector<char> source(len, 0);
    std::vector<char> buffer;
    buffer.resize(xtext::base64_length_from_binary(len) + 1);
    std::mt19937 gen((std::mt19937::result_type)(seed));
    std::uniform_int_distribution<int> byte_generator{0, 255};
    for (size_t trial = 0; trial < 10; trial++) {
      for (size_t i = 0; i < len; i++) {
      ...
      if (padding == 1) {
        ...
        xtext::result r = xtext::base64_to_binary(
            buffer.data(), buffer.size(), back.data());
        ASSERT_EQUAL(r.error, xtext::error_code::INVALID_BASE64_CHARACTER);
      } else if (padding == 2) {
        // adding padding should break
        {
          ...
          xtext::result r =
              xtext::base64_to_binary(copy.data(), copy.size(), back.data());
          ASSERT_EQUAL(r.error, xtext::error_code::INVALID_BASE64_CHARACTER);
        }
  ...
  for (size_t len = 0; len < max_len; len++) {
    ...
    for (size_t trial = 0; trial < 10; trial++) {
      ...
      std::vector<char> back(xtext::maximal_binary_length_from_base64(
          buffer.data(), buffer.size()));
      xtext::result r =
          xtext::base64_to_binary(buffer.data(), buffer.size(), back.data());
      ASSERT_EQUAL(r.error, xtext::error_code::INVALID_BASE64_CHARACTER);
      ...
      for (auto option :
           {xtext::last_chunk_handling_options::strict,
            ...
            xtext::last_chunk_handling_options::stop_before_partial}) {
        size_t back_length = back.size();
        r = xtext::base64_to_binary_safe(buffer.data(), buffer.size(),
...
TEST(doomed_truncated_base64_roundtrip) {
  for (size_t len = 1; len < max_len; len++) {
    ...
    for (size_t trial = 0; trial < 10; trial++) {
      ...
      std::vector<char> back(xtext::maximal_binary_length_from_base64(
          buffer.data(), buffer.size()));
      for (auto option : {xtext::last_chunk_handling_options::loose,
                          xtext::last_chunk_handling_options::strict}) {
        xtext::result r = implementation.base64_to_binary(
            buffer.data(), buffer.size(), back.data(), xtext::base64_default,
            option);
...
TEST(doomed_truncated_base64_roundtrip_16) {
  for (size_t len = 1; len < max_len; len++) {
    ...
    for (size_t trial = 0; trial < 10; trial++) {
      ...
      ASSERT_EQUAL(r.error, xtext::error_code::BASE64_INPUT_REMAINDER);
      size_t back_length = back.size();
      r = xtext::base64_to_binary_safe(buffer16.data(), buffer16.size(),
                                         back.data(), back_length);
      ASSERT_EQUAL(r.error, xtext::error_code::BASE64_INPUT_REMAINDER);
    }
  }
...
TEST(roundtrip_base64_16_with_garbage) {
  for (size_t len = 0; len < max_len; len++) {
    ...
    for (size_t trial = 0; trial < 10; trial++) {
      ...
      xtext::result r = implementation.base64_to_binary(
          buffer16.data(), buffer16.size(), back.data(),
          xtext::base64_default_accept_garbage);
      ASSERT_EQUAL(r.error, xtext::error_code::SUCCESS);
      ASSERT_EQUAL(r.count, len);
      if (back != source) {
        printf("=====input size %zu\n", len);
...
TEST(roundtrip_base64_url_16_with_garbage) {
  for (size_t len = 0; len < max_len; len++) {
    ...
    for (size_t trial = 0; trial < 10; trial++) {
      ...
      xtext::result r = implementation.base64_to_binary(
          buffer16.data(), buffer16.size(), back.data(),
          xtext::base64_url_accept_garbage);
      ASSERT_EQUAL(r.error, xtext::error_code::SUCCESS);
      ASSERT_EQUAL(r.count, len);
      if (back != source) {
        printf("=====input size %zu\n", len);
  ...
  for (size_t window = 16; window <= 2048; window += 7) {
    ...
    for (; pos < buffer.size();) {
      ...
      xtext::full_result r = implementation.base64_to_binary_details(
          ...
          xtext::base64_default,
          xtext::last_chunk_handling_options::only_full_chunks);
      ASSERT_EQUAL(r.error, xtext::error_code::SUCCESS);
...
TEST(readme_test) {
  ...
  for (size_t pos = 0; pos < base64.size(); pos += window) {
    ...
    xtext::result r = xtext::base64_to_binary(base64.data() + pos, count,
                                                  back.data() + outpos);
    if (r.error == xtext::error_code::INVALID_BASE64_CHARACTER) {
    ...
    if (count + pos == base64.size() &&
        r.error == xtext::error_code::BASE64_INPUT_REMAINDER) {
    ...
    if (r.error == xtext::error_code::BASE64_INPUT_REMAINDER) {
...
TEST(readme_safe) {
  ...
  r = xtext::base64_to_binary_safe(base64.data() + input_index,
...
TEST(base64_to_binary_safe_span_api_char) {
  ...
  const auto [ret, outlen] = xtext::base64_to_binary_safe(input, output);
  ASSERT_EQUAL(ret.error, xtext::SUCCESS);
...
TEST(binary_to_base64_with_lines_span_api_char) {
  ...
  std::string output(expected_output.size() + 4, '\0');
  const auto outlen = xtext::binary_to_base64_with_lines(input, output, 5);
  ASSERT_EQUAL(outlen,
...
TEST(binary_length_from_base64_basic) {
  ...
  ASSERT_EQUAL(xtext::binary_length_from_base64("YQ==", 4), 1);
  // "YWI=" decodes to "ab" (2 bytes)
  ASSERT_EQUAL(xtext::binary_length_from_base64("YWI=", 4), 2);
  // "YWJj" decodes to "abc" (3 bytes)
  ASSERT_EQUAL(xtext::binary_length_from_base64("YWJj", 4), 3);
  // "YWJjZA==" decodes to "abcd" (4 bytes)
  ASSERT_EQUAL(xtext::binary_length_from_base64("YWJjZA==", 8), 4);
  // Empty string
  ASSERT_EQUAL(xtext::binary_length_from_base64("", 0), 0);
}
...
TEST(binary_length_from_base64_with_spaces) {
  ...
  ASSERT_EQUAL(xtext::binary_length_from_base64("Y Q = =", 7), 1);
  // "  Y  Q  =  =  " with lots of spaces
  ASSERT_EQUAL(xtext::binary_length_from_base64("  Y  Q  =  =  ", 14), 1);
  // "  A  A  " should decode to 1 byte (2 base64 chars = 1 byte)
  ASSERT_EQUAL(xtext::binary_length_from_base64("  A  A  ", 8), 1);
  // "  A  A  G  A  /  v  8  " should decode to 5 bytes (7 base64 chars)
  ASSERT_EQUAL(
      xtext::binary_length_from_base64("  A  A  G  A  /  v  8  ", 23), 5);
}
...
TEST(binary_length_from_base64_matches_decode_result) {
  ...
  for (const auto &input : test_inputs) {
    size_t exact_len =
        xtext::binary_length_from_base64(input.data(), input.size());
    size_t max_len =
        xtext::maximal_binary_length_from_base64(input.data(), input.size());
    ...
    auto result =
        xtext::base64_to_binary(input.data(), input.size(), buffer.data());
    if (result.error == xtext::error_code::SUCCESS) {
...
TEST(binary_length_from_base64_char16) {
  ...
  ASSERT_EQUAL(
      xtext::binary_length_from_base64(input16.data(), input16.size()), 3);

  std::u16string input16_spaces = u"  Y  Q  =  =  ";
  ASSERT_EQUAL(xtext::binary_length_from_base64(input16_spaces.data(),
                                                  input16_spaces.size()),
               1);
...
TEST(binary_length_from_base64_url_variant) {
  ...
  ASSERT_EQUAL(
      xtext::binary_length_from_base64(url_input.data(), url_input.size()),
      3);
  ...
  ASSERT_EQUAL(
      xtext::binary_length_from_base64(url_input2.data(), url_input2.size()),
      4);
}
...
TEST(binary_length_from_base64_various_remainders) {
  ...
  ASSERT_EQUAL(xtext::binary_length_from_base64("A", 1), 0);
  // 2 base64 chars = 1 byte
  ASSERT_EQUAL(xtext::binary_length_from_base64("AA", 2), 1);
  // 3 base64 chars = 2 bytes
  ASSERT_EQUAL(xtext::binary_length_from_base64("AAA", 3), 2);
  // 4 base64 chars = 3 bytes
  ASSERT_EQUAL(xtext::binary_length_from_base64("AAAA", 4), 3);
  // 5 base64 chars = 3 bytes (remainder 1 contributes 0)
  ASSERT_EQUAL(xtext::binary_length_from_base64("AAAAA", 5), 3);
  // 6 base64 chars = 4 bytes
  ASSERT_EQUAL(xtext::binary_length_from_base64("AAAAAA", 6), 4);
  // 7 base64 chars = 5 bytes
  ASSERT_EQUAL(xtext::binary_length_from_base64("AAAAAAA", 7), 5);
  // 8 base64 chars = 6 bytes
  ASSERT_EQUAL(xtext::binary_length_from_base64("AAAAAAAA", 8), 6);
}
...
TEST(base64_details_padding_error_consistency) {
  ...
  const auto opt = xtext::base64_default;
  const auto lco = xtext::last_chunk_handling_options::strict;

  const size_t maxbinary =
      implementation.maximal_binary_length_from_base64(input, input_len);
  std::vector<char> output(maxbinary);

  const xtext::full_result fr = implementation.base64_to_binary_details(
      input, input_len, output.data(), opt, lco);

  ASSERT_TRUE(fr.error != xtext::error_code::SUCCESS);
...
TEST(base64_details_padding_error_various_inputs) {
  ...
  for (const char *input : cases) {
    ...
    const xtext::result r = implementation.base64_to_binary(
        input, input_len, output2.data(), opt, lco);

    const xtext::result r_from_fr = static_cast<xtext::result>(fr);
    ASSERT_EQUAL(r.error, r_from_fr.error);
    ASSERT_EQUAL(r.count, r_from_fr.count);

    // On success, output bytes must match.
    if (fr.error == xtext::error_code::SUCCESS) {
...
TEST(base64_details_input_count_on_padding_error) {
  ...
  const auto opt = xtext::base64_default_no_padding;
  const auto lco = xtext::last_chunk_handling_options::loose;
  ...
  ASSERT_TRUE(fr.error == xtext::error_code::INVALID_BASE64_CHARACTER);
  ASSERT_TRUE(fr.padding_error);
...
TEST(base64_to_binary_details_basic) {
  // Test the free function xtext::base64_to_binary_details
  const std::string base64_input = "QWJyYWNhZGFicmEh"; // "Abracadabra!"
  const std::string expected = "Abracadabra!";
  std::vector<char> output(xtext::maximal_binary_length_from_base64(
      base64_input.data(), base64_input.size()));
  xtext::full_result r = xtext::base64_to_binary_details(
...
TEST(base64_to_binary_details_utf16) {
  // Test the free function xtext::base64_to_binary_details with char16_t
...
TEST(base64_valid_runtime) {
  ...
  for (char c = 'A'; c <= 'Z'; c++) {
    ASSERT_TRUE(xtext::base64_valid(c));
  }
  for (char c = 'a'; c <= 'z'; c++) {
    ASSERT_TRUE(xtext::base64_valid(c));
  }
  ...
  // base64url
  ASSERT_TRUE(xtext::base64_valid('-', xtext::base64_url));
  ASSERT_TRUE(xtext::base64_valid('_', xtext::base64_url));
}

int main(int argc, char *argv[]) {
  const auto cmdline = xtext::test::CommandLine::parse(argc, argv);
  seed = cmdline.seed;

  xtext::test::run(cmdline);
