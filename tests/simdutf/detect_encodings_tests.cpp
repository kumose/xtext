...
TEST(issue818) {
  ...
  const auto r = xtext::BOM::check_bom(data.data(), data.size());
  ASSERT_EQUAL(r, xtext::encoding_type::UTF8);
}
...
TEST_LOOP(pure_utf8_ASCII) {
  ...
  for (size_t size : input_size) {
    ...
    auto expected =
        xtext::encoding_type::UTF8 | xtext::encoding_type::UTF16_LE;
...
TEST_LOOP(pure_utf16_ASCII) {
  xtext::tests::helpers::RandomInt random(0, 127, seed);
...
TEST_LOOP(pure_utf32_ASCII) {
  xtext::tests::helpers::RandomInt random(0, 0x7f, seed);

  for (size_t size : input_size) {
    const auto generated = generate_u32(random, size / 4);
    auto expected = xtext::encoding_type::UTF8 |
                    xtext::encoding_type::UTF16_LE |
...
TEST_LOOP(no_utf8_bytes_no_surrogates) {
  xtext::tests::helpers::RandomIntRanges random(
      {{0x007f, 0xd800 - 1}, {0xe000, 0xffff}}, seed);

  for (size_t size : input_size) {
    auto generated = generate_u32(random, size / 4);
    if (!xtext::match_system(xtext::endianness::LITTLE)) {
    ...
    auto expected =
        xtext::encoding_type::UTF16_LE | xtext::encoding_type::UTF32_LE;
    auto actual = implementation.detect_encodings(
        reinterpret_cast<const char *>(generated.data()), size);
    ASSERT_EQUAL((actual & expected),
...
TEST_LOOP(two_utf8_bytes) {
  xtext::tests::helpers::random_utf8 random(seed, 0, 1, 0, 0);
  ...
  for (size_t size : input_size) {
    ...
    auto actual = implementation.detect_encodings(
        reinterpret_cast<const char *>(generated.data()), size);
    if (actual != expected) {
      if ((actual & xtext::encoding_type::UTF8) == 0) {
        puts("failed to detect valid UTF-8.");
      }
      if ((actual & xtext::encoding_type::UTF16_LE) == 0) {
...
TEST_LOOP(utf_16_surrogates) {
  ...
  for (size_t size : input_size) {
    ...
    auto expected = xtext::encoding_type::UTF16_LE;
    auto actual = implementation.detect_encodings(
        reinterpret_cast<const char *>(generated.data()), size);
...
TEST_LOOP(utf32_surrogates) {
  xtext::tests::helpers::RandomInt random_prefix(0x10000, 0x10ffff, seed);
  xtext::tests::helpers::RandomInt random_suffix(0xd800, 0xdfff, seed);
  ...
  for (size_t size : input_size) {
    ...
    auto expected = xtext::encoding_type::UTF32_LE;
    auto actual = implementation.detect_encodings(
        reinterpret_cast<const char *>(generated.data()), size);
...
TEST_LOOP(edge_surrogate) {
  xtext::tests::helpers::RandomInt random(0x10000, 0x10ffff, seed);
  ...
  while (i + 32 < (size / 2) - 1) {
    char16_t W1;
    char16_t W2;
    ASSERT_EQUAL(xtext::tests::reference::utf16::encode(random(), W1, W2), 2);
...
TEST_LOOP(tail_utf8) {
  xtext::tests::helpers::random_utf8 random(seed, 0, 0, 1, 0);
