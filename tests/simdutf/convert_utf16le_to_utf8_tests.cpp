...
constexpr std::array<size_t, 9> input_size{7,   12,  16,  64,  67,
                                           128, 256, 511, 1000};
constexpr xtext::endianness LE = xtext::endianness::LITTLE;

using xtext::tests::helpers::transcode_utf16_to_utf8_test_base;
...
TEST_LOOP(convert_into_1_or_2_UTF8_bytes) {
  xtext::tests::helpers::RandomInt random(
...
TEST(all_possible_8_codepoint_combinations) {
  ...
  for (const auto &input_utf16 : combinations) {
    ...
    if (xtext::tests::reference::validate_utf16(LE, input_utf16.data(),
                                                  input_utf16.size())) {
...
template <auto input> constexpr auto convert() {
  ...
  constexpr auto Nout = xtext::utf8_length_from_utf16(input);
  CTString<char8_t, Nout> tmp{};
  std::size_t N;
  if constexpr (decltype(input)::endianness == std::endian::native) {
    N = xtext::convert_utf16_to_utf8(input, tmp);
  }
  if (N != tmp.size()) {
    throw "oops";
  }
