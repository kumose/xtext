...
TEST(issue_631) {
  ...
  ASSERT_EQUAL(validation1.error, xtext::error_code::TOO_LONG);

  const bool validation2 =
      implementation.validate_utf8((const char *)data, data_len);
  ASSERT_EQUAL(validation1.error == xtext::error_code::SUCCESS, validation2);
...
TEST(convert_check_validation) {
  uint32_t seed{1234};
  xtext::tests::helpers::random_utf8 gen_1_2_3_4(seed, 1, 1, 1, 1);
  size_t total = 1000;
    ...
    for (size_t flip = 0; flip < 1000; ++flip) {
      ...
      bool is_ok =
          (implementation.convert_utf8_to_utf16le(
               (const char *)UTF8.data(), UTF8.size(), buffer.get()) > 0);
      bool is_ok_reference = xtext::tests::reference::validate_utf8(
...
TEST_LOOP(convert_2_UTF8_bytes) {
  xtext::tests::helpers::RandomInt random(0x0080, 0x07ff,
...
TEST_LOOP(convert_3_UTF8_bytes) {
  xtext::tests::helpers::RandomInt random(0x0800, 0xd800 - 1,
...
static void test_corrupt(T &implementation, uint32_t seed,
                         xtext::tests::helpers::random_utf8 gen_utf8) {
  ...
  for (size_t i = 0; i < 10; i++) {
    ...
    for (size_t j = 0; j < 1000; ++j) {
      ...
      bool is_ok =
          (implementation.convert_utf8_to_utf16le(
               (const char *)UTF8.data(), UTF8.size(), buffer.get()) > 0);
      bool is_ok_basic = xtext::tests::reference::validate_utf8(
          (const char *)UTF8.data(), UTF8.size());
...
TEST(corrupt_1byte) {
  ...
  test_corrupt(implementation, seed,
               xtext::tests::helpers::random_utf8(seed, 1, 0, 0, 0));
}

TEST(corrupt_12byte) {
  uint32_t seed{1234};
  test_corrupt(implementation, seed,
               xtext::tests::helpers::random_utf8(seed, 0, 1, 0, 0));
  test_corrupt(implementation, seed,
               xtext::tests::helpers::random_utf8(seed, 1, 1, 0, 0));
}

TEST(corrupt_123byte) {
  uint32_t seed{1234};
  test_corrupt(implementation, seed,
               xtext::tests::helpers::random_utf8(seed, 0, 0, 1, 0));
  test_corrupt(implementation, seed,
               xtext::tests::helpers::random_utf8(seed, 0, 1, 1, 0));
  test_corrupt(implementation, seed,
               xtext::tests::helpers::random_utf8(seed, 1, 0, 1, 0));
  test_corrupt(implementation, seed,
               xtext::tests::helpers::random_utf8(seed, 1, 1, 1, 0));
}
...
template <auto input> constexpr auto test_compile() {
  ...
  auto ret = xtext::convert_utf8_to_utf16(input, tmp);
  return ret;
}
} // namespace
