...
using test_procedure = void (*)(const xtext::implementation &impl);
struct test_entry {
  std::string name;
  test_procedure procedure;

  void operator()(const xtext::implementation &impl);
};
...
#ifndef XTEXT_SIMDUTF_TEST_LOOP_TRIALS
  #error "XTEXT_SIMDUTF_TEST_LOOP_TRIALS not set."
#endif
constexpr std::size_t trials = XTEXT_SIMDUTF_TEST_LOOP_TRIALS;
...
#define TEST(name)                                                             \
  void test_impl_##name(const xtext::implementation &impl);                  \
  void name(const xtext::implementation &impl) {                             \
    xtext::get_active_implementation() = &impl;                              \
    test_impl_##name(impl);                                                    \
  }                                                                            \
  static xtext::test::register_test test_register_##name(#name, name);       \
  void test_impl_##name(                                                       \
      xtext_maybe_unused const xtext::implementation &implementation)

#define TEST_LOOP(name)                                                        \
  void test_impl_##name(const xtext::implementation &impl, uint32_t seed);   \
  void name(const xtext::implementation &impl) {                             \
    for (size_t trial = 0; trial < xtext::test::trials; trial++) {           \
  ...
  static xtext::test::register_test test_register_##name(#name, name);       \
  void test_impl_##name(const xtext::implementation &implementation,         \
                        xtext_maybe_unused uint32_t seed)
...
#define TEST_MAIN                                                              \
  int main(int argc, char *argv[]) { return xtext::test::main(argc, argv); }
