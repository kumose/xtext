...
auto xtext::test::CommandLine::parse(int argc, char *argv[])
    -> xtext::test::CommandLine {
  CommandLine cmdline;
  cmdline.seed = 42;
...
void print_architectures(FILE *file) {
  ...
  for (const auto &implementation : xtext::get_available_implementations()) {
...
void print_tests(FILE *file) {
  ...
  for (const auto &test : xtext::test::test_procedures()) {
...
void test_entry::operator()(const xtext::implementation &impl) {
...
void run(const CommandLine &cmdline) {
  ...
  for (const auto &implementation : xtext::get_available_implementations()) {
    ...
    auto filter = [&cmdline](const xtext::test::test_entry &test) -> bool {
    ...
    for (auto test : xtext::test::test_procedures()) {
      if (filter(test)) {
        test(*implementation);
      }
    }
