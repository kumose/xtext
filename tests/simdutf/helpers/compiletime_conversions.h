#ifndef XTEXT_SIMDUTF_COMPILETIME_CONVERSIONS_H
#define XTEXT_SIMDUTF_COMPILETIME_CONVERSIONS_H
...
constexpr auto to_wellformed(const CTString<CharType, N, endianness> &input) {
  ...
  if constexpr (endianness == std::endian::little) {
    xtext::to_well_formed_utf16le(input, output);
  } else {
    xtext::to_well_formed_utf16be(input, output);
  }
  return output;
}
...
constexpr auto to_latin1(const CTString<CharType, N, endianness> &input) {
  ...
  if constexpr (std::is_same_v<CharType, char32_t>) {
    ret = xtext::convert_valid_utf32_to_latin1(input, tmp);
  } else if constexpr (std::is_same_v<CharType, char16_t> &&
                       endianness == std::endian::little) {
    ret = xtext::convert_valid_utf16le_to_latin1(input, tmp);
  } else if constexpr (std::is_same_v<CharType, char16_t> &&
                       endianness == std::endian::big) {
    ret = xtext::convert_valid_utf16be_to_latin1(input, tmp);
  } else {
    throw "unknown type";
  }
...
constexpr auto to_utf8() {
  ...
  auto ret = xtext::convert_latin1_to_utf8(input, tmp);
  if (!input.empty() && ret == 0) {
    throw "failed conversion";
  }
  return tmp;
...
to_utf16_impl(const CTString<CharType, N, input_endianness> &input) {
  ...
  if constexpr (std::is_same_v<CharType, char16_t>) {
    if constexpr (output_endianness == input_endianness) {
      ...
      xtext::change_endianness_utf16(input, output);
      return output;
    }
  }
}
...
constexpr auto utf8_to_utf16() {
  ...
  constexpr auto Nout = xtext::utf16_length_from_utf8(input);
  CTString<char16_t, Nout, target_endianness> tmp{};
  std::size_t N;
  if constexpr (target_endianness == std::endian::little) {
    if constexpr (with_errors) {
      auto res = xtext::convert_utf8_to_utf16le_with_errors(input, tmp);
      if (res.is_err()) {
        throw "fail";
      }
      N = res.count;
    } else {
      N = xtext::convert_utf8_to_utf16le(input, tmp);
    }
  } else {
    if constexpr (with_errors) {
      auto res = xtext::convert_utf8_to_utf16be_with_errors(input, tmp);
      if (res.is_err()) {
        throw "fail";
      }
      N = res.count;
    } else {
      N = xtext::convert_utf8_to_utf16be(input, tmp);
    }
  }
  if (N != input.size()) {
    throw "oops";
...
constexpr auto valid_utf8_to_utf16() {
  ...
  std::size_t N;
  if constexpr (target_endianness == std::endian::little) {
    N = xtext::convert_valid_utf8_to_utf16le(input, tmp);
  } else {
    N = xtext::convert_valid_utf8_to_utf16be(input, tmp);
  }
  if (N != input.size()) {
    throw "oops";
  }
...
template <auto input> constexpr auto utf8_length_from_utf16_helper() {
  if constexpr (decltype(input)::endianness == std::endian::little) {
    return xtext::utf8_length_from_utf16le(input);
  } else {
    return xtext::utf8_length_from_utf16be(input);
  }
}
} // namespace detail
...
template <auto input, bool with_errors> constexpr auto valid_utf16_to_utf8() {
  ...
  std::size_t N;
  if constexpr (decltype(input)::endianness == std::endian::little) {
    N = xtext::convert_valid_utf16le_to_utf8(input, tmp);
  } else {
    N = xtext::convert_valid_utf16be_to_utf8(input, tmp);
  }
  if (N != input.size()) {
    throw "oops";
  }
...
template <auto input, bool with_errors> constexpr auto utf16_to_utf8() {
  ...
  std::size_t N;
  if constexpr (decltype(input)::endianness == std::endian::little) {
    N = xtext::convert_utf16le_to_utf8(input, tmp);
  } else {
    N = xtext::convert_utf16be_to_utf8(input, tmp);
  }
  if (N == 0) {
    throw "failed";
  }
...
#endif // XTEXT_SIMDUTF_CPLUSPLUS23

#endif // XTEXT_SIMDUTF_COMPILETIME_CONVERSIONS_H
